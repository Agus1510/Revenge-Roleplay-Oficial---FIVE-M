...
function permBanUser(bannedBy, id)
	bannedTable[id] = {
		banner = bannedBy,
		reason = "Fuiste baneado permanentemente",
		expire = 0
	}
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "ban" then
							...
							DropPlayer(id, GetConvar("es_admin_banreason", "Fuiste baneado del servidor"))
						end
					else
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget then
						...
						table.remove(reason, 1)
						if(#reason == 0)then
							reason = "Fuiste baneado del servidor"
						else
						...
						TriggerClientEvent('chat:addMessage', -1, {
							args = {"^1SYSTEM", "Player ^2" .. GetPlayerName(player) .. "^0 Fue kickeado(^2" .. reason .. "^0)"}
						})
...
TriggerEvent('es:addGroupCommand', 'anuncio', "admin", function(source, args, user)
	TriggerClientEvent('chat:addMessage', -1, {
		args = {"^1ANUNCIO", table.concat(args, " ")}
	})
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = {"^1SYSTEM", "Insufficienct permissions!"} })
end, {help = "Mensaje a todo el servidor", params = {{name = "anuncio", help = "El mensaje para anuncio"}}})
--//
-- Freezing
local frozen = {}
