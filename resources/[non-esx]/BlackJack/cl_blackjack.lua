...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(1014.68, 47.35, 73.28),
        dealerHeading = 287.71,
        tablePos = vector3(1015.583, 47.48, 72.27),
        tableHeading = 103.303,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [1] = {	
		dealerPos = vector3(1023.77,61.51, 72.48),
        dealerHeading = 12.27,
        tablePos = vector3(1023.64, 62.1, 72.48),
        tableHeading = 194.77,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
...
Citizen.CreateThread(function()
    ...
    local blackjackTable = GetClosestObjectOfType(1024.84, 57.14, 73.88,1.0,GetHashKey("vw_prop_casino_blckjack_01b"),0,0,0)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Tu mano es ["..tostring(currentHand).."], La mano del dealer es ["..tostring(dealersHand).."]")
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "El objetivo del blacjack es vencer la mano del dealer sin pasar los 21. Este juego usa 2 mazos de 52 cartas cada uno, los cuales se mezclan antes de empezar cada mano..                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "El objetivo del blacjack es vencer la mano del dealer sin pasar los 21. Este juego usa 2 mazos de 52 cartas cada uno, los cuales se mezclan antes de empezar cada ronda.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Esperando a que empieze el proximo juego...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("~b~Te pasaste!")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
