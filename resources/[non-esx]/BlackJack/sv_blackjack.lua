...
function tryTakeChips(source,amount)
	...
	local item = xPlayer.getInventoryItem('Moneda_Revenge').count
		if item >= amount then
			xPlayer.removeInventoryItem('Moneda_Revenge',amount)
			return true
		else 
			TriggerClientEvent('tac:showNotification', source, 'No tienes suficientes fichas')
			return false --you dont actually need to return false, since default is false but anw. I like this way
		end
end
function giveChips(source,amount)
    local xPlayer = tac.GetPlayerFromId(source)
	xPlayer.addInventoryItem('Moneda_Revenge', amount)
end
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if source ~= nil then
        ...
        TriggerClientEvent("blackjack:notify",source,"~r~No puedes sentarte.")
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if tryTakeChips(source,betAmount) then
                        ...
                        TriggerClientEvent("blackjack:notify",source,"~r~No tienes suficientes fichas!")
                    end
                end
            end
        end
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "Diamond Casino | " .. GetPlayerName(source) .. " Ganó " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " fichas!")
                                                            end
                                                        end
                                                    end
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            if potentialPushAmount > 10000000 then
                                                                TriggerClientEvent('chatMessage', -1, "Diamond Casino | " .. GetPlayerName(source) .. " Perdio " .. tostring(getMoneyStringFormatted(potentialPushAmount)) .. " fichas!")
                                                            end
                                                        end
                                                    end
                                                end
