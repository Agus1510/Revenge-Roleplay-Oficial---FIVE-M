...
Citizen.CreateThread(function()
	while tac == nil do TriggerEvent('tac:getSharedObject', function(obj) tac = obj end) Wait(0) end
    while not NetworkIsSessionStarted() or tac.GetPlayerData().job == nil do Wait(0) end
    while true do
		...
		then
            ...
            if trunk ~= -1 then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords, true) <= 1.5 then
                    if not inTrunk then
                        if GetVehicleDoorAngleRatio(vehicle, 5) < 0.9 then
                            DrawText3D(coords, '[E] Esconderse [H] Abrir')
								if IsControlJustReleased(0, 74)then
									if lockStatus == 1 then --unlocked
										SetCarBootOpen(vehicle)
									elseif lockStatus == 2 then -- locked
										tac.ShowNotification('El auto esta cerrado')
									end
								end
                        else
                            DrawText3D(coords, '[E] Esconderse [H] Abrir')
                    ...
                    if IsControlJustReleased(0, 38) and not inTrunk then
                        local player = tac.Game.GetClosestPlayer()
						...
						if lockStatus == 1 then --unlocked
							if DoesEntityExist(playerPed) then
								if not IsEntityAttached(playerPed) or GetDistanceBetweenCoords(GetEntityCoords(playerPed), GetEntityCoords(PlayerPedId()), true) >= 5.0 then
									...
									tac.ShowNotification('Ya hay alguien en el baul!')
								end
							end
						elseif lockStatus == 2 then -- locked
							tac.ShowNotification('El auto esta cerrado')
