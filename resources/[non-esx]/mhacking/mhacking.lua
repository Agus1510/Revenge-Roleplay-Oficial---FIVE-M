	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navega con ~y~W,A,S,D~s~ y confirma con ~y~SPACE~s~ por el bloque de la izquierda.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Usa las ~y~Flechas~s~ y ~y~ENTER~s~ para el bloque derecho")
			else
