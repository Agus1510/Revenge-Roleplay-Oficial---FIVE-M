    ...
    while true do
        Citizen.Wait(0)
			...
			for k in pairs(locations) do
				...
				tac.ShowHelpNotification('Presiona ~INPUT_PICKUP~ para atrapar cangrejos/camarones')
			...
			if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						...
						DrawText3Ds(vector3(Config.Processing.x,Config.Processing.y,Config.Processing.z), "Presiona [E] para empaquetar cangrejos ", 0.35)
...
------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				if GetDistanceBetweenCoords(Config.camarones.x, Config.camarones.y, Config.camarones.z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(Config.camarones.x, Config.camarones.y, Config.camarones.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						---Draw3DText( Config.Processing.x, Config.Processing.y, Config.Processing.z , "~w~Produccion de cangrejos~y~\nPresiona [~b~E~y~] para comenzar el empaquetado",4,0.15,0.1)
						DrawText3Ds(vector3(Config.camarones.x,Config.camarones.y,Config.camarones.z), "Presiona [E] para empaquetar camarones ", 0.35)
...
local making = true
	...
	TriggerEvent('tac:showNotification', '~g~Comenzando ~w~la produccion ~g~de cangrejos')
	Citizen.Wait(5000)
	
	tac.TriggerServerCallback('Farmcyc:empaquetado:cangrejos', function(output)
			making = output
		end)
...
local makingcamarones = true
	...
	TriggerEvent('tac:showNotification', '~g~Comenzando ~w~la produccion ~g~de camarones')
	Citizen.Wait(5000)
	
	tac.TriggerServerCallback('Farmcyc:empaquetado:camarones', function(output)
...
Citizen.Wait(1)
   ...
   if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.ventacyc.x, Config.ventacyc.y, Config.ventacyc.z, true) < 2 then
   	DrawText3Ds(vector3(Config.ventacyc.x,Config.ventacyc.y,Config.ventacyc.z), "Presiona [E] para abrir el menu de tienda ", 0.35)


   	--tac.ShowHelplNotification("Presiona ~INPUT_CONTEXT~ para abrir el menu de venta")
...
local elements ={  
{ label = 'Vender Camarones <span style = "color:#65D754;">' ..Config.PCamaron ..  '$</span>', value = 'Camarones1'},

{ label = 'Vender Cangrejos <span style = "color:#65D754;">' ..Config.PCangrejo ..  '$</span>', value = 'Cangrejos1'},
...
tac.UI.Menu.Open('default', GetCurrentResourceName(),'ventacyc',{
	title = 'ventacyc', 
	align = 'top-left',
	elements = elements  
