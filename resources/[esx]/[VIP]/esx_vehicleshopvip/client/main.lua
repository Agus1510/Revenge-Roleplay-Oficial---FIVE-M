...
Citizen.CreateThread(function()
	...
	tac.TriggerServerCallback('tac_vehicleshopvip:getCategories', function(categories)
		Categories = categories
	end)

	tac.TriggerServerCallback('tac_vehicleshopvip:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)

	if Config.EnablePlayerManagement then
		if tac.PlayerData.job.name == 'cardealervip' then
			Config.Zones.ShopEntering.Type = 1

			if tac.PlayerData.job.grade_name == 'boss' then
...
RegisterNetEvent('tac_vehicleshopvip:sendCategories')
AddEventHandler('tac_vehicleshopvip:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('tac_vehicleshopvip:sendVehicles')
AddEventHandler('tac_vehicleshopvip:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function DeleteDisplayVehicleInsideShop()
	...
	if currentDisplayVehicle and DoesEntityExist(currentDisplayVehicle) then
		...
		if DoesEntityExist(currentDisplayVehicle) and NetworkHasControlOfEntity(currentDisplayVehicle) then
			tac.Game.DeleteVehicle(currentDisplayVehicle)
		end
	end
end

function ReturnVehicleProvider()
	tac.TriggerServerCallback('tac_vehicleshopvip:getCommercialVehicles', function(vehicles)
		local elements = {}

		for k,v in ipairs(vehicles) do
			local returnPrice = tac.Math.Round(v.price * 0.75)
			local vehicleLabel = getVehicleLabelFromModel(v.vehicle)

			table.insert(elements, {
				label = ('%s [<span style="color:orange;">%s</span>]'):format(vehicleLabel, _U('generic_shopitem', tac.Math.GroupDigits(returnPrice))),
				value = v.vehicle
			})
		end

		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			title    = _U('return_provider_menu'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			TriggerServerEvent('tac_vehicleshopvip:returnProvider', data.current.value)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[tac_vehicleshopvip] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[tac_vehicleshopvip] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	for i=1, #Categories, 1 do
		...
		for j=1, #categoryVehicles, 1 do
			...
			table.insert(options, ('%s <span style="color:green;">%s</span>'):format(vehicle.name, _U('generic_shopitem', tac.Math.GroupDigits(vehicle.price))))
		end
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			title = _U('buy_vehicle_shop', vehicleData.name, tac.Math.GroupDigits(vehicleData.price)),
			align = 'top-left',
			elements = {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					tac.TriggerServerCallback('tac_vehicleshopvip:buycardealervipVehicle', function(success)
						if success then
							IsInShopMenu = false
							...
							menu2.close()
							menu.close()
							tac.ShowNotification(_U('vehicle_purchased'))
						else
							tac.ShowNotification(_U('broke_company'))
						end
					end, vehicleData.model)
				else
					local generatedPlate = GeneratePlate()

					tac.TriggerServerCallback('tac_vehicleshopvip:buyVehicle', function(success)
						if success then
							IsInShopMenu = false
							menu2.close()
							menu.close()
							...
							tac.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
		...
		tac.Game.SpawnLocalVehicle(vehicleData.model, Config.Zones.ShopInside.Pos, Config.Zones.ShopInside.Heading, function(vehicle)
	...
	tac.Game.SpawnLocalVehicle(firstVehicleData.model, Config.Zones.ShopInside.Pos, Config.Zones.ShopInside.Heading, function(vehicle)
		currentDisplayVehicle = vehicle
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
...
function OpenResellerMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
		title    = _U('car_dealer'),
		align    = 'top-left',
		elements = {
		...
		elseif action == 'depop_vehicle' then
			if currentDisplayVehicle then
				...
				tac.ShowNotification(_U('no_current_vehicle'))
			end
		...
		elseif action == 'create_bill' then
			...
			if closestPlayer ~= -1 and closestDistance < 3 then
				tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('tac_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealervip', _U('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					local vehicleProps = tac.Game.GetVehicleProperties(currentDisplayVehicle)
					vehicleProps.plate = newPlate
					SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
					TriggerServerEvent('tac_vehicleshopvip:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
		...
		elseif action == 'set_vehicle_owner_rent' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
						...
						if not amount then
							...
							if closestPlayer ~= -1 and closestDistance < 3 then
								local newPlate = 'RENT' .. string.upper(tac.GetRandomString(4))
								local model = CurrentVehicleData.model
								SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
								TriggerServerEvent('tac_vehicleshopvip:rentVehicle', model, newPlate, amount, GetPlayerServerId(closestPlayer))
								currentDisplayVehicle = nil
							else
								tac.ShowNotification(_U('no_players'))
							end
...
function OpenPopVehicleMenu()
	tac.TriggerServerCallback('tac_vehicleshopvip:getCommercialVehicles', function(vehicles)
		...
		for k,v in ipairs(vehicles) do
			...
			table.insert(elements, {
				label = ('%s [MSRP <span style="color:green;">%s</span>]'):format(vehicleLabel, _U('generic_shopitem', tac.Math.GroupDigits(v.price))),
				value = v.vehicle
			})
		end

		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'commercial_vehicles', {
			...
			tac.Game.SpawnVehicle(model, Config.Zones.ShopInside.Pos, Config.Zones.ShopInside.Heading, function(vehicle)
				currentDisplayVehicle = vehicle
...
function OpenRentedVehiclesMenu()
	tac.TriggerServerCallback('tac_vehicleshopvip:getRentedVehicles', function(vehicles)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'rented_vehicles', {
...
function OpenBossActionsMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('tac_society:openBossMenu', 'cardealervip', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			tac.TriggerServerCallback('tac_vehicleshopvip:getSoldVehicles', function(customers)
				local elements = {
				...
				tac.UI.Menu.Open('list', GetCurrentResourceName(), 'sold_vehicles', elements, function(data2, menu2)
...
function OpenGetStocksMenu()
	tac.TriggerServerCallback('tac_vehicleshopvip:getStockItems', function(items)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('tac_vehicleshopvip:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	tac.TriggerServerCallback('tac_vehicleshopvip:getPlayerInventory', function(inventory)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('tac_vehicleshopvip:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('tac_vehicleshopvip:hasEnteredMarker', function(zone)
	if zone == 'ShopEntering' then

		if Config.EnablePlayerManagement then
			if tac.PlayerData.job ~= nil and tac.PlayerData.job.name == 'cardealervip' then
				CurrentAction     = 'reseller_menu'
				CurrentActionMsg  = _U('shop_menu')
				CurrentActionData = {}
			end
	...
	elseif zone == 'ResellVehicle' then
		...
		if IsPedSittingInAnyVehicle(playerPed) then
			...
			if GetPedInVehicleSeat(vehicle, -1) == playerPed then
				...
				resellPrice = tac.Math.Round(vehicleData.price / 100 * Config.ResellPercentage)
				model = GetEntityModel(vehicle)
				plate = tac.Math.Trim(GetVehicleNumberPlateText(vehicle))

				CurrentAction     = 'resell_vehicle'
				CurrentActionMsg  = _U('sell_menu', vehicleData.name, tac.Math.GroupDigits(resellPrice))
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and tac.PlayerData.job ~= nil and tac.PlayerData.job.name == 'cardealervip' and tac.PlayerData.job.grade_name == 'boss' then
		CurrentAction     = 'boss_actions_menu'
		CurrentActionMsg  = _U('shop_menu')
		CurrentActionData = {}
	end
...
AddEventHandler('tac_vehicleshopvip:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('tac_vehicleshopvip:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_vehicleshopvip:hasExitedMarker', LastZone)
		end
			...
			if IsControlJustReleased(0, 38) then
				if CurrentAction == 'shop_menu' then
					if Config.LicenseEnable then
						tac.TriggerServerCallback('tac_license:checkLicense', function(hasDriversLicense)
							if hasDriversLicense then
									OpenShopMenu()
							else
								tac.ShowNotification(_U('license_missing'))
							end
				...
				elseif CurrentAction == 'give_back_vehicle' then
					tac.TriggerServerCallback('tac_vehicleshopvip:giveBackVehicle', function(isRentedVehicle)
						if isRentedVehicle then
							tac.Game.DeleteVehicle(CurrentActionData.vehicle)
							tac.ShowNotification(_U('delivered'))
						else
							tac.ShowNotification(_U('not_rental'))
						end
					end, tac.Math.Trim(GetVehicleNumberPlateText(CurrentActionData.vehicle)))
				elseif CurrentAction == 'resell_vehicle' then
					tac.TriggerServerCallback('tac_vehicleshopvip:resellVehicle', function(vehicleSold)
						if vehicleSold then
							tac.Game.DeleteVehicle(CurrentActionData.vehicle)
							tac.ShowNotification(_U('vehicle_sold_for', CurrentActionData.label, tac.Math.GroupDigits(CurrentActionData.price)))
						else
							tac.ShowNotification(_U('not_yours'))
						end
