...
tac.RegisterServerCallback('pxrp_vip:getVIPStatus', function(source, cb)
...
TriggerEvent('es:addGroupCommand', 'addvipstatus', 'superadmin', function(source, args, user)
	if args[1] ~= nil then
		if GetPlayerName(tonumber(args[1])) ~= nil then
			...
			template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(158, 35, 35, 0.4); border-radius: 3px;"><i class="fas fa-globe"></i> <b>[Sistema] '..GetPlayerName(tonumber(args[1]))..'</b> <i>Ha sido agregado a la lista VIP.</i></div>'
			})			
