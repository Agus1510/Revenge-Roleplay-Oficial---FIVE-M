	...
	while true do
		...
		if (GetLastInputMethod(2) and IsControlJustPressed(1, 38) and IsControlPressed(1, 21)) and not IsPedInAnyVehicle(playerPed, true) then			
			...
			if sitting then
				...
				local object, distance = tac.Game.GetClosestObject(Config.Interactables)
...
function wakeup()
	...
	TriggerServerEvent('tac_interact:leavePlace', currentSitObj)
	currentSitObj = nil
	currentScenario = nil
end
...
function sit(object, modelName, data)
	...
	tac.TriggerServerCallback('tac_interact:getPlace', function(occupied)

		if occupied then
			tac.ShowNotification("Cette place est prise...")
		else
			local playerPed = GetPlayerPed(-1)
			lastPos = GetEntityCoords(playerPed)
			currentSitObj = id

			TriggerServerEvent('tac_interact:takePlace', id)
