...
RegisterServerEvent("tac_trunk_inventory:getOwnedVehicule")
AddEventHandler(
  "tac_trunk_inventory:getOwnedVehicule",
  function()
    local vehicules = {}
    ...
    MySQL.Async.fetchAll(
      ...
      function(result)
        ...
        TriggerClientEvent("tac_trunk_inventory:setOwnedVehicule", _source, vehicules)
      end
    )
  end
)
  ...
  function(source, cb, plate)
    TriggerEvent(
      ...
      function(store)
        ...
        for i = 1, #coffre, 1 do
          table.insert(items, {name = coffre[i].name, count = coffre[i].count, label = tac.GetItemLabel(coffre[i].name)})
        end
...
RegisterServerEvent("tac_trunk:getItem")
AddEventHandler(
  "tac_trunk:getItem",
      ...
      if targetItem.limit == nil or ((targetItem.count + count) <= targetItem.limit) then
        TriggerEvent(
          ...
          function(store)
            ...
            TriggerClientEvent("tac_inventoryhud:refreshTrunkInventory", _source, data, blackMoney, items, weapons)
          end
        )
      else
        TriggerClientEvent(
...
RegisterServerEvent("tac_trunk:putItem")
AddEventHandler(
  "tac_trunk:putItem",
