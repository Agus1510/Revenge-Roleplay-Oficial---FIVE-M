...
AddEventHandler('tac_rvphone:loaded', function(phoneNumber, contacts)
	...
	if count > 0 then
		...
		TriggerEvent('tac_rvphone:removeSpecialContact', specialContact.number)
	end
end)
...
RegisterNetEvent('tac_extraitems:oxygen_mask')
AddEventHandler('tac_extraitems:oxygen_mask', function()
	local playerPed  = GetPlayerPed(-1)
	local coords     = GetEntityCoords(playerPed)
	local boneIndex  = GetPedBoneIndex(playerPed, 12844)
	local boneIndex2 = GetPedBoneIndex(playerPed, 24818)

	tac.Game.SpawnObject('p_s_scuba_mask_s', {
		x = coords.x,
		y = coords.y,
		z = coords.z - 3
	}, function(object)
		tac.Game.SpawnObject('p_s_scuba_tank_s', {
			x = coords.x,
			y = coords.y,
			z = coords.z - 3
		}, function(object2)
			...
			tac.ShowNotification(_U('dive_suit_on') .. '%.')
			Citizen.Wait(50000)
			tac.ShowNotification(_U('oxygen_notify', '~y~', '50') .. '%.')
			Citizen.Wait(25000)
			tac.ShowNotification(_U('oxygen_notify', '~o~', '25') .. '%.')
			Citizen.Wait(25000)
			tac.ShowNotification(_U('oxygen_notify', '~r~', '0') .. '%.')
...
RegisterNetEvent('tac_extraitems:bulletproof')
AddEventHandler('tac_extraitems:bulletproof', function()
	local playerPed = GetPlayerPed(-1)
...
RegisterNetEvent('tac_extraitems:firstaidkit')
AddEventHandler('tac_extraitems:firstaidkit', function()
	local playerPed = GetPlayerPed(-1)
	local health = GetEntityHealth(playerPed)
	local max = GetEntityMaxHealth(playerPed)

	if health > 0 and health < max then
		tac.ShowNotification(_U('use_firstaidkit'))
...
RegisterNetEvent('tac_extraitems:clipcli')
AddEventHandler('tac_extraitems:clipcli', function()
	ped = GetPlayerPed(-1)
	if IsPedArmed(ped, 4) then
		hash = GetSelectedPedWeapon(ped)
		if hash ~= nil then
			AddAmmoToPed(GetPlayerPed(-1), hash,1000)
			tac.ShowNotification(_U("clip_use"))
		else
			tac.ShowNotification(_U("clip_no_weapon"))
		end
	else
		tac.ShowNotification(_U("clip_not_suitable"))
	end
end)
...
RegisterNetEvent('tac_extraitems:startDrill')
AddEventHandler('tac_extraitems:startDrill', function(source)
	local playerPed = GetPlayerPed(-1)
...
RegisterNetEvent('tac_extraitems:lockpick')
AddEventHandler('tac_extraitems:lockpick', function()
...
RegisterNetEvent('tac_extraitems:binoculars')
AddEventHandler('tac_extraitems:binoculars', function()
