...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        local players, nearbyPlayer = tac.Game.GetPlayersInArea(GetEntityCoords(playerPed), 3.0)
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("tac:removeInventoryItem", data.item.type, data.item.name, data.number)
        end
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if foundPlayer then
            ...
            TriggerServerEvent("tac:giveInventoryItem", data.player, data.item.type, data.item.name, count)
            Wait(500)
            loadPlayerInventory()
        else
            exports.pNotify:SendNotification(
