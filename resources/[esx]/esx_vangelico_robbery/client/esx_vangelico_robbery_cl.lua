...
RegisterNetEvent('tac_vangelico_robbery:currentlyrobbing')
AddEventHandler('tac_vangelico_robbery:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
end)

RegisterNetEvent('tac_vangelico_robbery:killblip')
AddEventHandler('tac_vangelico_robbery:killblip', function()
    RemoveBlip(blipRobbery)
end)

RegisterNetEvent('tac_vangelico_robbery:setblip')
AddEventHandler('tac_vangelico_robbery:setblip', function(position)
...
RegisterNetEvent('tac_vangelico_robbery:toofarlocal')
AddEventHandler('tac_vangelico_robbery:toofarlocal', function(robb)
	holdingup = false
...
RegisterNetEvent('tac_vangelico_robbery:robberycomplete')
AddEventHandler('tac_vangelico_robbery:robberycomplete', function(robb)
	holdingup = false
	tac.ShowNotification(_U('robbery_complete'))
	store = ""
	incircle = false
end)
	...
	while true do
						...
						if IsPedShooting(GetPlayerPed(-1)) then
							if Config.NeedBag then
							    if borsa == 40 or borsa == 41 or borsa == 44 or borsa == 45 then
							        tac.TriggerServerCallback('tac_vangelico_robbery:conteggio', function(CopsConnected)
								        if CopsConnected >= Config.RequiredCopsRob then
							                TriggerServerEvent('tac_vangelico_robbery:rob', k)
									        PlaySoundFromCoord(soundid, "VEHICLES_HORNS_AMBULANCE_WARNING", pos2.x, pos2.y, pos2.z)
								        else
									        TriggerEvent('tac:showNotification', _U('min_two_police') .. Config.RequiredCopsRob .. _U('min_two_police2'))
								        end
							        end)		
						        else
							        TriggerEvent('tac:showNotification', _U('need_bag'))
								end
							else
								tac.TriggerServerCallback('tac_vangelico_robbery:conteggio', function(CopsConnected)
									if CopsConnected >= Config.RequiredCopsRob then
		...
		if holdingup then
			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					...
					if IsControlJustPressed(0, 38) then
					    ...
					    TriggerServerEvent('tac_vangelico_robbery:gioielli')
						...
						if vetrineRotte == Config.MaxWindows then 
							...
							TriggerServerEvent('tac_vangelico_robbery:endrob', store)
						    tac.ShowNotification(_U('lester'))
						    holdingup = false
						    StopSound(soundid)
						end
					end
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -622.566, -230.183, 38.057, true) > 11.5 ) then
				TriggerServerEvent('tac_vangelico_robbery:toofar', store)
				holdingup = false
				for i,v in pairs(vetrine) do 
					v.isOpen = false
					vetrineRotte = 0
							...
							if quantity >= Config.MaxJewelsSell then
								tac.TriggerServerCallback('tac_vangelico_robbery:conteggio', function(CopsConnected)
									if CopsConnected >= Config.RequiredCopsSell then
										...
										blip = false
										TriggerEvent('tac:showNotification', _U('copsforsell') .. Config.RequiredCopsSell .. _U('copsforsell2'))
									end
								end)
							else
								blip = false
								TriggerEvent('tac:showNotification', _U('notenoughgold'))
							end
						end, 'jewels')
					end
				end
