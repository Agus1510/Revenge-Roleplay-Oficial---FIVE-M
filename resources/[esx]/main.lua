        ...
        if data.current.value == 'interactions_menu' then
            tac.UI.Menu.Open('default', GetCurrentResourceName(), 'interactions_menu', {
                ...
                if data.current.value == 'my_phone_number' then
                        TriggerServerEvent('tac_aiomenu:givePhoneNumber', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()))
                        tac.UI.Menu.CloseAll()
                elseif data.current.value == 'give_phone_number' then
                    ...
                    elseif closestPlayer ~= -1 and closestDistance <= 2.0 then
                        TriggerServerEvent('tac_aiomenu:givePhoneNumber', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer))
                    end

                    tac.UI.Menu.CloseAll()
                elseif data.current.value == 'my_id' then
                    TriggerServerEvent('tac_aiomenu:showID', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()))
                    tac.UI.Menu.CloseAll()					
                elseif data.current.value == 'show_id' then
                    ...
                    if closestPlayer == -1 or closestDistance > 2.0 then
                        tac.ShowNotification(_U('no_players'))
                    elseif closestPlayer ~= -1 and closestDistance <= 2.0 then
                        TriggerServerEvent('tac_aiomenu:showID', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer))
                        tac.UI.Menu.CloseAll()
                    end
                 end
                                    ...
                                    elseif data.current.value == 'toggle_trunk' then
                                        ...
                                        if vehicle ~= nil and vehicle ~= 0 and GetPedInVehicleSeat(vehicle, 0) then
                                            ...
                                            if boot ~= -1 then
                                                ...
                                                tac.ShowNotification('Este vehiculo no tiene trunk.')
                                            end
                                        else
                                            tac.ShowNotification('Debes ser el conductor para hacer esto.')
                                        end
