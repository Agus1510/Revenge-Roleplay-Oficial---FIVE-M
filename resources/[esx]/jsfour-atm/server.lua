...
-- Get money
tac.RegisterServerCallback('jsfour-atm:getMoney', function(source, cb)
...
-- Get user info
tac.RegisterServerCallback('jsfour-atm:getUser', function(source, cb)
  local _source = source
	local identifier = tac.GetPlayerFromId(_source).identifier
  local userData = {}
...
-- Check item
tac.RegisterServerCallback('jsfour-atm:item', function(source, cb)
...
AddEventHandler('jsfour-atm:insert', function(amount)
	...
	if amount > xPlayer.getMoney() then
		...
		TriggerClientEvent('tac:showNotification', _source, 'Depositaste $' .. amount .. '~s~')
	end
end)
...
AddEventHandler('jsfour-atm:take', function(amount)
	...
	if amount > accountMoney then
		...
		TriggerClientEvent('tac:showNotification', _source, 'Extraiste $' .. amount .. '~s~ ')
	end
end)
...
AddEventHandler('jsfour-atm:transfer', function(amount, receiver)
  ...
  function (result)
    if (result[1] ~= nil) then
      local recPlayer    = tac.GetPlayerFromIdentifier(result[1].identifier)
      local senPlayer    = tac.GetPlayerFromId(_source)
        ...
        function (result)
          if (result[1] ~= nil) then
            TriggerClientEvent('tac:showNotification', _source, 'Enviaste $' .. amount .. '~s~ a ' .. result[1].firstname .. ' ' .. result[1].lastname)
            TriggerClientEvent('tac:showNotification', recPlayer.source, 'Recibiste $' .. amount .. '~s~ enviado por ' .. result[1].firstname .. ' ' .. result[1].lastname)
          end
        end)
      end
    end
...
AddEventHandler('jsfour-atm:createAccount', function( src )
  ...
  if src == nil then
    identifier = tac.GetPlayerFromId(_source).identifier
  else
    identifier = tac.GetPlayerFromId(src).identifier
  end
