...
AddEventHandler('trew_hud_ui:setInfo', function(info)
	...
	if tac.PlayerData.job ~= nil then
		if tac.PlayerData.job.grade_name ~= nil and tac.PlayerData.job.grade_name == 'boss' then
		...
		while true do
			...
			if IsControlJustPressed(1, Keys[Config.voice.keys.distance]) then
				...
				elseif Config.voice.levels.current == 1 then
					NetworkSetTalkerProximity(Config.voice.levels.gritar)
					voiceDistance = 'gritar'
