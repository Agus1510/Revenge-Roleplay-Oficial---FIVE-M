RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" then
		...
		tac.ShowNotification("No eres policia!")
	end
end)
...
function Cutscene()
	...
	tac.TriggerServerCallback('tac_skin:getPlayerSkin', function(skin, jailSkin)
		if skin.sex == 0 then
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Security" then
				...
				if PlayerData.job.name ~= "police" then
					tac.ShowNotification("No tienes la llave para entrar aqui!")
					return
				end
			end
