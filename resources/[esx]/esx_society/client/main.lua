...
function RefreshBussHUD()
	...
	if tac.PlayerData.job.grade_name == 'boss' then
		...
		tac.TriggerServerCallback('tac_society:getSocietyMoney', function(money)
			UpdateSocietyMoneyHUDElement(money)
		end, tac.PlayerData.job.name)
	end
end

RegisterNetEvent('tac_addonaccount:setMoney')
AddEventHandler('tac_addonaccount:setMoney', function(society, money)
	if tac.PlayerData.job and tac.PlayerData.job.grade_name == 'boss' and 'society_' .. tac.PlayerData.job.name == society then
		UpdateSocietyMoneyHUDElement(money)
	end
end)
...
function EnableSocietyMoneyHUDElement()
	...
	if tac.GetConfig().EnableHud then
		tac.UI.HUD.RegisterElement('society_money', 3, 0, societyMoneyHUDElementTpl, {
			money = 0
		})
	end
...
function DisableSocietyMoneyHUDElement()
	if tac.GetConfig().EnableHud then
		tac.UI.HUD.RemoveElement('society_money')
	end
...
function UpdateSocietyMoneyHUDElement(money)
	if tac.GetConfig().EnableHud then
		tac.UI.HUD.UpdateElement('society_money', {
			money = tac.Math.GroupDigits(money)
		})
	end
...
function OpenBossMenu(society, close, options)
	...
	tac.TriggerServerCallback('tac_society:isBoss', function(result)
		isBoss = result
	end, society)
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
		title    = _U('boss_menu'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		...
		if data.current.value == 'withdraw_society_money' then
			...
			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
		...
		elseif data.current.value == 'deposit_money' then
			...
			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_' .. society, {
		...
		elseif data.current.value == 'wash_money' then
			...
			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_' .. society, {
...
function OpenManageEmployeesMenu(society)
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees_' .. society, {
...
function OpenEmployeeList(society)
	...
	tac.TriggerServerCallback('tac_society:getEmployees', function(employees)
		...
		tac.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
				tac.ShowNotification(_U('you_have_fired', employee.name))

				tac.TriggerServerCallback('tac_society:setJob', function()
					OpenEmployeeList(society)
...
function OpenRecruitMenu(society)
	...
	tac.TriggerServerCallback('tac_society:getOnlinePlayers', function(players)
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. society, {
			title    = _U('recruiting'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)

			tac.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
				...
				if data2.current.value == 'yes' then
					tac.ShowNotification(_U('you_have_hired', data.current.name))
...
function OpenPromoteMenu(society, employee)
	...
	tac.TriggerServerCallback('tac_society:getJob', function(job)
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_employee_' .. society, {
			title    = _U('promote_employee', employee.name),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			menu.close()
			tac.ShowNotification(_U('you_have_promoted', employee.name, data.current.label))
...
function OpenManageGradesMenu(society)
	...
	tac.TriggerServerCallback('tac_society:getJob', function(job)
		...
		for i=1, #job.grades, 1 do
			...
			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(gradeLabel, _U('money_generic', tac.Math.GroupDigits(job.grades[i].salary))),
				value = job.grades[i].grade
			})
		end

		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_grades_' .. society, {
			title    = _U('salary_management'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)

			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_grades_amount_' .. society, {
				...
				elseif amount > Config.MaxSalary then
					tac.ShowNotification(_U('invalid_amount_max'))
				else
					menu2.close()

					tac.TriggerServerCallback('tac_society:setJobSalary', function()
						OpenManageGradesMenu(society)
