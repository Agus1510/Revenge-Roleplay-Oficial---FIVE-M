...
Citizen.CreateThread(function() -- tags  
    while true do
        ...
        local ped = PlayerPedId()
            local yate = AddBlipForCoord(-2084.88, -1018.12, 12.78)
                SetBlipSprite(yate, 160)
                BeginTextCommandSetBlipName("STRING")
                AddTextComponentString(_U('blipname'))
                SetBlipAsShortRange(yate, true)
                EndTextCommandSetBlipName(yate)
                return
        end
    end)
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), -2084.88, -1018.12, 12.78, true) < 50 then
            DrawMarker(35, -2084.95, -1018.08, 12.78, 0, 0, 0, 0, 0, 75.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 1, 0, 2, 0, 0, 0, 0)
                if GetDistanceBetweenCoords(GetEntityCoords(ped), -2084.88, -1018.12, 12.78, true) < 1 then
                    tac.ShowHelpNotification(_U('start_robbery'))
                        if IsControlJustReleased(1, 51) then
                            TriggerServerEvent("tac_yate:robbery")
                        end
                    end
                end
            end
...
RegisterNetEvent("tac_yate:start")
AddEventHandler("tac_yate:start", function()
  heist = true
  tags = true
  Citizen.CreateThread(function()
    while true do 
      Citizen.Wait(1)
        ...
        if location1 == false and tags == true then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), -2054.18, -1032.38, 8.97, true) < 1 then
                tac.ShowHelpNotification(_U('search_robbery'))
                    if IsControlJustReleased(1, 51) then
                        Animacja()
                        location1 = true
                        searched = searched+1
...
Citizen.CreateThread(function()
    while searched < 20 do 
        Citizen.Wait(100)
            if searched == 20 then
                TriggerServerEvent("tac_yate:reward")
                heist = false
                tags = false
                Citizen.Wait(19000)
                tac.ShowHelpNotification(_U('suitcase_found'))
            end
        end
    end)
...
RegisterNetEvent("tac_yate:notification")
AddEventHandler("tac_yate:notification", function(text)
    tac.ShowHelpNotification(text)
end)

RegisterNetEvent("tac_yate:notify")
AddEventHandler("tac_yate:notify", function(text)
    tac.ShowNotification(_U('notification'))
