...
TAC.TriggerClientCallback = function(source, name, cb, ...)
    ...
    TriggerClientEvent('revengeanticheat:triggerClientCallback', source, name, TAC.ClientCallbacks[playerId]['CurrentRequestId'], ...)
...
TAC.TriggerClientEvent = function(source, name, ...)
    TriggerClientEvent('revengeanticheat:triggerClientEvent', source, name, ...)
end
...
TAC.TriggerServerCallback = function(name, source, cb, ...)
    if (TAC.ServerCallbacks ~= nil and TAC.ServerCallbacks[name] ~= nil) then
        ...
        print('[revengeAntiCheat] TriggerServerCallback => ' .. _('callback_not_found', name))
    end
end
...
TAC.TriggerServerEvent = function(name, source, ...)
    if (TAC.ServerEvents ~= nil and TAC.ServerEvents[name] ~= nil) then
        ...
        print('[revengeAntiCheat] TriggerServerEvent => ' .. _('trigger_not_found', name))
    end
end

RegisterServerEvent('revengeanticheat:clientCallback')
AddEventHandler('revengeanticheat:clientCallback', function(requestId, ...)
