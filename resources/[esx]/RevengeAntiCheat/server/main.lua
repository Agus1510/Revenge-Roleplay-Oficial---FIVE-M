...
TAC.LoadConfig = function()
    ...
    TAC.Config = {
        UpdateIdentifiers   = TAC.GetConfigVariable('revengeanticheat.updateidentifiers', 'boolean'),
        GodMode             = TAC.GetConfigVariable('revengeanticheat.godmode', 'boolean'),
        Webhook             = TAC.GetConfigVariable('revengeanticheat.webhook', 'string'),
        BypassEnabled       = TAC.GetConfigVariable('revengeanticheat.bypassenabled', 'boolean'),
        VPNCheck            = TAC.GetConfigVariable('revengeanticheat.VPNCheck', 'boolean', true),
        VPNKey              = TAC.GetConfigVariable('revengeanticheat.VPNKey', 'string')
    }
...
TAC.LogBanToDiscord = function (data)
    ...
    local discordInfo = {
        ...
        ["footer"] = {
            ["text"] = 'revengeAntiCheat | ' .. TAC.Version
        }
    }

    PerformHttpRequest(TAC.Config.Webhook, function(err, text, headers) end, 'POST', json.encode({ username = 'revengeAntiCheat', embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
TAC.RegisterServerCallback('revengeanticheat:getServerConfig', function(source, cb)
    while not TAC.ConfigLoaded do
        Citizen.Wait(10)
    end
...
TAC.RegisterServerCallback('revengeanticheat:getRegisteredCommands', function(source, cb)
    cb(GetRegisteredCommands())
end)

TAC.RegisterServerEvent('revengeanticheat:banPlayer', function(source, type, item)
...
TAC.RegisterServerEvent('revengeanticheat:playerResourceStarted', function(source)
...
TAC.RegisterServerEvent('revengeanticheat:logToConsole', function(source, message)
    print(message)
end)
