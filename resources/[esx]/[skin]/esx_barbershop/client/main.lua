	...
	TriggerEvent('tac_skin:openRestrictedMenu', function(data, menu)
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data.current.value == 'yes' then
				tac.TriggerServerCallback('tac_barbershop:checkMoney', function(hasEnoughMoney)
					if hasEnoughMoney then
						TriggerEvent('skinchanger:getSkin', function(skin)
							TriggerServerEvent('tac_skin:save', skin)
						end)

						TriggerServerEvent('tac_barbershop:pay')
						hasPaid = true
					else
...
AddEventHandler('tac_barbershop:hasEnteredMarker', function(zone)
	currentAction = 'shop_menu'
	currentActionMsg = _U('press_access')
end)

AddEventHandler('tac_barbershop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not hasAlreadyEnteredMarker) or (isInMarker and lastZone ~= currentZone) then
			...
			TriggerEvent('tac_barbershop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('tac_barbershop:hasExitedMarker', lastZone)
		end
