...
function SetUnsetAccessory(accessory)
	tac.TriggerServerCallback('tac_accessories:get', function(hasAccessory, accessorySkin)
	...
	TriggerEvent('tac_skin:openRestrictedMenu', function(data, menu)
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm',
		{
			...
			if data.current.value == 'yes' then
				tac.TriggerServerCallback('tac_accessories:checkMoney', function(hasEnoughMoney)
					if hasEnoughMoney then
						TriggerServerEvent('tac_accessories:pay')
						TriggerEvent('skinchanger:getSkin', function(skin)
							TriggerServerEvent('tac_accessories:save', skin, accessory)
						end)
					else
...
AddEventHandler('tac_accessories:hasEnteredMarker', function(zone)
	CurrentAction     = 'shop_menu'
	CurrentActionMsg  = _U('press_access')
	CurrentActionData = { accessory = zone }
end)

AddEventHandler('tac_accessories:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('tac_accessories:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_accessories:hasExitedMarker', LastZone)
		end
