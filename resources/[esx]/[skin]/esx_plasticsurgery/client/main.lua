...
function OpenPlasticSurgery()
	...
	TriggerEvent('tac_skin:openSaveableMenu', function(data, menu)
		menu.close()
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data.current.value == 'yes' then
				tac.TriggerServerCallback('tac_plasticsurgery:checkMoney', function(hasEnoughMoney)
					if hasEnoughMoney then
						TriggerEvent('skinchanger:getSkin', function(skin)
							TriggerServerEvent('tac_skin:save', skin)
						end)
						
						TriggerServerEvent('tac_plasticsurgery:pay')
						HasPaid = true
					else
...
AddEventHandler('tac_plasticsurgery:hasEnteredMarker', function(zone)
	CurrentAction     = 'surgery_menu'
	CurrentActionMsg  = _U('press_access')
	CurrentActionData = {}
end)

AddEventHandler('tac_plasticsurgery:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('tac_plasticsurgery:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_plasticsurgery:hasExitedMarker', LastZone)
		end
...
function refreshBlips()
	if Config.EnableBlips then
		if Config.EnableUnemployedOnly then
			if tac.PlayerData.job ~= nil and tac.PlayerData.job.name == 'unemployed' or tac.PlayerData.job ~= nil and tac.PlayerData.job.name == 'gang' then
