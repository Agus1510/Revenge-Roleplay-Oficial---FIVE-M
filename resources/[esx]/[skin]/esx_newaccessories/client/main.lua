...
function OpenAccessoryMenu()
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'set_unset_accessory',
	{
		title = _U('set_unset'),
		align = 'top-right',
		elements = {
		...
		if data.current.value ~= 'Helmet' and data.current.value ~= 'Ears' and data.current.value ~= 'Mask' and data.current.value ~= 'Glasses' then
			...
			elseif data.current.value == 'shirt' then
				TriggerEvent('tac_newaccessories:shirt')
				tac.UI.Menu.CloseAll()	
			elseif data.current.value == 'pants' then
				TriggerEvent('tac_newaccessories:pants')
				tac.UI.Menu.CloseAll()	
			elseif data.current.value == 'shoes' then
				TriggerEvent('tac_newaccessories:shoes')
				tac.UI.Menu.CloseAll()	
			end
		else
...
RegisterNetEvent('tac_newaccessories:shirt')
AddEventHandler('tac_newaccessories:shirt', function()
...
RegisterNetEvent('tac_newaccessories:pants')
AddEventHandler('tac_newaccessories:pants', function()
	TriggerEvent('skinchanger:getSkin', function(skin)
		local clothesSkin = {
...
RegisterNetEvent('tac_newaccessories:shoes')
AddEventHandler('tac_newaccessories:shoes', function()
	TriggerEvent('skinchanger:getSkin', function(skin)
		--[[local clothesSkin = {
...
function SetUnsetAccessory(accessory)
	tac.TriggerServerCallback('tac_newaccessories:get', function(hasAccessory, accessorySkin)
		...
		if hasAccessory then
			...
			tac.ShowNotification(_U('no_' .. _accessory))
		end
...
function OpenShopMenu(accessory)
	...
	TriggerEvent("tac_np_skinshop:toggleMenu", _accessory)
...
function openEndDialog()
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm',
	{
		...
		if data.current.value == 'yes' then
			tac.TriggerServerCallback('tac_newaccessories:checkMoney', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('tac_newaccessories:pay')
					TriggerEvent('skinchanger:getSkin', function(skin)
						TriggerServerEvent('tac_newaccessories:save', skin, accesorio) --accessory
					end)
					hasPaid = true
				else
...
RegisterNetEvent('tac_newaccessories:endDialog')
AddEventHandler('tac_newaccessories:endDialog', function()
	openEndDialog()
end)
...
AddEventHandler('tac_newaccessories:hasEnteredMarker', function(zone)
	CurrentAction     = 'shop_menu'
	CurrentActionMsg  = _U('press_access')
	CurrentActionData = { accessory = zone }
end)

AddEventHandler('tac_newaccessories:hasExitedMarker', function(zone)
	--print('apretaste esc')
	tac.UI.Menu.CloseAll()
	CurrentAction = nil
	if not hasPaid then
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('tac_newaccessories:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_newaccessories:hasExitedMarker', LastZone)
		end
