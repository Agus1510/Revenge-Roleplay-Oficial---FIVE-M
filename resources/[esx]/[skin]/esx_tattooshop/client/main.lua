...
AddEventHandler('skinchanger:modelLoaded', function()
	tac.TriggerServerCallback('tac_tattooshop:requestPlayerTattoos', function(tattooList)
		if tattooList then
...
function OpenShopMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'tattoo_shop', {
		title    = _U('tattoos'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		...
		if data.current.value then
			...
			for k,v in pairs(Config.TattooList[data.current.value]) do
				table.insert(elements, {
					label = _U('tattoo_item', k, _U('money_amount', tac.Math.GroupDigits(v.price))),
					value = k,
					price = v.price
				})
			end

			tac.UI.Menu.Open('default', GetCurrentResourceName(), 'tattoo_shop_categories', {
				...
				if data2.current.value ~= nil then
					...
					tac.TriggerServerCallback('tac_tattooshop:purchaseTattoo', function(success)
						if success then
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('tac_tattooshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_tattooshop:hasExitedMarker', LastZone)
		end
	end
end)

AddEventHandler('tac_tattooshop:hasEnteredMarker', function(zone)
...
AddEventHandler('tac_tattooshop:hasExitedMarker', function(zone)
	CurrentAction = nil
