...
Citizen.CreateThread(function()
    while true do
        ...
        if GetDistanceBetweenCoords(coords, Config.Zones.Shop.coords, true) < 3.0 then
            tac.ShowHelpNotification(_U('shop_prompt'))
...
function OpenShop()
    ...
    for k, v in pairs(tac.GetPlayerData().inventory) do
        ...
        if price and v.count > 0 then
            table.insert(elements, {
                label = ('%s - <span style="color:green;">%s</span>'):format(v.label, _U('item', tac.Math.GroupDigits(price))),
                name = v.name,
                price = price,
    ...
    tac.UI.Menu.Open('default', GetCurrentResourceName(), 'car_shop', {
        title    = _U('shop_title'),
        align    = 'right',
        elements = elements
    }, function(data, menu)
...
function ChopVehicle()
    ...
    elseif
        ...
        tac.TriggerServerCallback('Lenzh_chopshop:anycops', function(anycops)
            if anycops >= Config.CopsRequired then
                ...
                tac.ShowNotification(_U('not_enough_cops'))
            end
        end)
    else
...
AddEventHandler('ChopNotify', function(alert)
    if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
        tac.ShowAdvancedNotification(_U('911'), _U('chop'), _U('call'), 'CHAR_CALL911', 7)
