...
AddEventHandler('tac:playerLoaded', function(xPlayer)
	if Config.UsePrivateCarGarages == true then
		tac.TriggerServerCallback('tac_advancedgarage:getOwnedProperties', function(properties)
...
function OpenMenuGarage(PointType)
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'garage_menu', {
		title    = _U('garage'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
...
function ListOwnedCarsMenu()
	...
	tac.TriggerServerCallback('tac_advancedgarage:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
			tac.ShowNotification(_U('garage_nocars'))
		else
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value.stored then
				...
				tac.ShowNotification(_U('car_is_impounded'))
			end
		end, function(data, menu)
			menu.close()
		end)
...
function ListOwnedBoatsMenu()
	...
	tac.TriggerServerCallback('tac_advancedgarage:getOwnedBoats', function(ownedBoats)
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_boat', {
			...
			if data.current.value.stored then
				...
				tac.ShowNotification(_U('boat_is_impounded'))
			end
		end, function(data, menu)
			menu.close()
		end)
...
function ListOwnedAircraftsMenu()
	...
	tac.TriggerServerCallback('tac_advancedgarage:getOwnedAircrafts', function(ownedAircrafts)
		if #ownedAircrafts == 0 then
			tac.ShowNotification(_U('garage_noaircrafts'))
		else
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_aircraft', {
			...
			if data.current.value.stored then
				...
				tac.ShowNotification(_U('aircraft_is_impounded'))
			end
		end, function(data, menu)
			menu.close()
		end)
...
function StoreOwnedCarsMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		tac.TriggerServerCallback('tac_advancedgarage:storeVehicle', function(valid)
			if valid then
				...
				tac.ShowNotification(_U('cannot_store_vehicle'))
			end
		end, vehicleProps)
	else
		tac.ShowNotification(_U('no_vehicle_to_enter'))
	end
end
...
function ReturnOwnedCarsMenu()
	tac.TriggerServerCallback('tac_advancedgarage:getOutOwnedCars', function(ownedCars)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_car', {
			title    = _U('pound_cars'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			tac.TriggerServerCallback('tac_advancedgarage:checkMoneyCars', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('tac_advancedgarage:payCar')
...
function ReturnOwnedBoatsMenu()
	tac.TriggerServerCallback('tac_advancedgarage:getOutOwnedBoats', function(ownedBoats)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_boat', {
			title    = _U('pound_boats'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			tac.TriggerServerCallback('tac_advancedgarage:checkMoneyBoats', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('tac_advancedgarage:payBoat')
					SpawnPoundedVehicle(data.current.value, data.current.value.plate)
				else
					tac.ShowNotification(_U('not_enough_money'))
				end
...
function ReturnOwnedAircraftsMenu()
	tac.TriggerServerCallback('tac_advancedgarage:getOutOwnedAircrafts', function(ownedAircrafts)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_aircraft', {
			title    = _U('pound_aircrafts'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			tac.TriggerServerCallback('tac_advancedgarage:checkMoneyAircrafts', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('tac_advancedgarage:payAircraft')
					SpawnPoundedVehicle(data.current.value, data.current.value.plate)
				else
					tac.ShowNotification(_U('not_enough_money'))
				end
...
function ReturnOwnedPolicingMenu()
	tac.TriggerServerCallback('tac_advancedgarage:getOutOwnedPolicingCars', function(ownedPolicingCars)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_policing', {
			title    = _U('pound_police'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			tac.TriggerServerCallback('tac_advancedgarage:checkMoneyPolicing', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('tac_advancedgarage:payPolicing')
					SpawnPoundedVehicle(data.current.value, data.current.value.plate)
				else
					tac.ShowNotification(_U('not_enough_money'))
				end
...
function ReturnOwnedAmbulanceMenu()
	tac.TriggerServerCallback('tac_advancedgarage:getOutOwnedAmbulanceCars', function(ownedAmbulanceCars)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_ambulance', {
			title    = _U('pound_ambulance'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			tac.TriggerServerCallback('tac_advancedgarage:checkMoneyAmbulance', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('tac_advancedgarage:payAmbulance')
					SpawnPoundedVehicle(data.current.value, data.current.value.plate)
				else
					tac.ShowNotification(_U('not_enough_money'))
				end
...
function RepairVehicle(apprasial, vehicle, vehicleProps)
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
		...
		if data.current.value == 'yes' then
			TriggerServerEvent('tac_advancedgarage:payhealth', apprasial)
			vehicleProps.bodyHealth = 1000.0 -- must be a decimal value!!!
			vehicleProps.engineHealth = 1000
			StoreVehicle(vehicle, vehicleProps)
		elseif data.current.value == 'no' then
			tac.ShowNotification(_U('visit_mechanic'))
		end
	end, function(data, menu)
		menu.close()
	end)
...
function StoreVehicle(vehicle, vehicleProps)
	...
	TriggerServerEvent('tac_advancedgarage:setVehicleState', vehicleProps.plate, true)
	tac.ShowNotification(_U('vehicle_in_garage'))
end

-- Spawn Vehicles
function SpawnVehicle(vehicle, plate)
	tac.Game.SpawnVehicle(vehicle.model, {
		...
		tac.Game.SetVehicleProperties(callback_vehicle, vehicle)
		SetVehRadioStation(callback_vehicle, "OFF")
		TaskWarpPedIntoVehicle(GetPlayerPed(-1), callback_vehicle, -1)
	end)

	TriggerServerEvent('tac_advancedgarage:setVehicleState', plate, false)
end
...
-- Entered Marker
AddEventHandler('tac_advancedgarage:hasEnteredMarker', function(zone)
...
-- Exited Marker
AddEventHandler('tac_advancedgarage:hasExitedMarker', function()
	tac.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if Config.UseJobCarGarages then
			if tac.PlayerData.job ~= nil and tac.PlayerData.job.name == 'police' then
			...
			if tac.PlayerData.job ~= nil and tac.PlayerData.job.name == 'ambulance' then
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerEvent('tac_advancedgarage:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('tac_advancedgarage:hasExitedMarker', LastZone)
		end
