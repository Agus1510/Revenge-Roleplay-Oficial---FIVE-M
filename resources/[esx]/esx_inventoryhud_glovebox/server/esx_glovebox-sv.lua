...
RegisterServerEvent("tac_glovebox_inventory:getOwnedVehicle")
AddEventHandler(
  "tac_glovebox_inventory:getOwnedVehicle",
  function()
    local vehicules = {}
    ...
    MySQL.Async.fetchAll(
      ...
      function(result)
        ...
        TriggerClientEvent("tac_glovebox_inventory:setOwnedVehicle", _source, vehicules)
      end
    )
  end
)
...
function getTotalInventoryWeight(plate)
  local total
  TriggerEvent(
    "tac_glovebox:getSharedDataStore",
    plate,
    function(store)
    ...
    TriggerEvent(
      ...
      function(store)
        ...
        for i = 1, #coffres, 1 do
          table.insert(items, {name = coffres[i].name, count = coffres[i].count, label = tac.GetItemLabel(coffres[i].name)})
        end
...
RegisterServerEvent("tac_glovebox:getItem")
AddEventHandler(
  "tac_glovebox:getItem",
      ...
      if targetItem.limit == nil or ((targetItem.count + count) <= targetItem.limit) then
        TriggerEvent(
          ...
          function(store)
            ...
            TriggerClientEvent("tac_inventoryhud:refreshGloveboxInventory", _source, data, blackMoney, items, weapons)
          end
        )
      else
        TriggerClientEvent(
...
RegisterServerEvent("tac_glovebox:putItem")
AddEventHandler(
  "tac_glovebox:putItem",
...
tac.RegisterServerCallback(
  "tac_glovebox:getPlayerInventory",
  function(source, cb)
