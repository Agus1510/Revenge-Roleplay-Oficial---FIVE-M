...
function PlayerKilledByPlayer(killerServerId, killerClientId, killerWeapon)
	...
	local data = {
		victimCoords = { x = tac.Math.Round(victimCoords.x, 1), y = tac.Math.Round(victimCoords.y, 1), z = tac.Math.Round(victimCoords.z, 1) },
		killerCoords = { x = tac.Math.Round(killerCoords.x, 1), y = tac.Math.Round(killerCoords.y, 1), z = tac.Math.Round(killerCoords.z, 1) },

		killedByPlayer = true,
		deathCause     = killerWeapon,
		distance       = tac.Math.Round(distance, 1),

		killerServerId = killerServerId,
		killerClientId = killerClientId
	}

	TriggerEvent('tac:onPlayerDeath', data)
	TriggerServerEvent('tac:onPlayerDeath', data)
end
