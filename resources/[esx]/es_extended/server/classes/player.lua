function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
	...
	self.getCoords = function(vectorType)
		...
		coords = {x = tac.Math.Round(coords.x, 1), y = tac.Math.Round(coords.y, 1), z = tac.Math.Round(coords.z, 1)}
	...
	self.getLastPosition = function()
		if self.lastPosition and self.lastPosition.x and self.lastPosition.y and self.lastPosition.z then
			self.lastPosition.x = tac.Math.Round(self.lastPosition.x, 1)
			self.lastPosition.y = tac.Math.Round(self.lastPosition.y, 1)
			self.lastPosition.z = tac.Math.Round(self.lastPosition.z, 1)
		end
	...
	self.setAccountMoney = function(acc, money)
		...
		local newMoney  = tac.Math.Round(money)
	...
	self.addAccountMoney = function(acc, money)
		...
		local account  = self.getAccount(acc)
		local newMoney = account.money + tac.Math.Round(money)
	...
	self.canCarryItem = function(name, count)
		local currentWeight, itemWeight = self.getWeight(), tac.Items[name].weight
	...
	self.setMaxWeight = function(newWeight)
		...
		TriggerClientEvent('tac:setMaxWeight', self.source, self.maxWeight)
	end
	...
	self.addWeapon = function(weaponName, ammo)
		local weaponLabel = tac.GetWeaponLabel(weaponName)
	...
	self.addWeaponAmmo = function(weaponName, ammoCount)
		...
		if weapon then
			...
			TriggerClientEvent('tac:setWeaponAmmo', self.source, weaponName, weapon.ammo)
		end
	end
	...
	self.showNotification = function(msg)
		TriggerClientEvent('tac:showNotification', self.source, msg)
	end

	self.showHelpNotification = function(msg)
		TriggerClientEvent('tac:showHelpNotification', self.source, msg)
	end
