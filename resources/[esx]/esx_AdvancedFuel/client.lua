						...
						while done == false do
							...
							if(amountToEssence-0.0005 > 0) then
								...
								if(_essence > 0.142) then
									essence = 0.142
									TriggerEvent("advanceFuel:setEssence", 100, GetVehicleNumberPlateText(veh), GetDisplayNameFromVehicleModel(GetEntityModel(veh)))
									done = true
								end
...
RegisterNetEvent("advanceFuel:setEssence")
AddEventHandler("advanceFuel:setEssence", function(percent, plate, model)
	...
	TriggerServerEvent("advanceFuel:setEssence_s",percent,plate,model)
end)
