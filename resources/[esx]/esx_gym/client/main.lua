...
RegisterNetEvent('tac_gym:useBandage')
AddEventHandler('tac_gym:useBandage', function()
	...
	tac.ShowNotification("Usaste una ~g~bandage")
end)

RegisterNetEvent('tac_gym:trueMembership')
AddEventHandler('tac_gym:trueMembership', function()
	membership = true
end)

RegisterNetEvent('tac_gym:falseMembership')
AddEventHandler('tac_gym:falseMembership', function()
	membership = false
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presiona ~INPUT_CONTEXT~ para alquilar una ~b~bici')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						tac.ShowNotification("Ya tienes un vehiculo")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presiona ~INPUT_CONTEXT~ para abrir el menu del ~b~gym~w~')
            ...
            if dist <= 0.5 then
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						TriggerServerEvent('tac_gym:checkChip')
						tac.ShowNotification("Preparando el ~g~ejercicio~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							tac.ShowNotification("Debes descansar ~r~60 segundos ~w~antes de hacer otro ejercicio.")
							
							--TriggerServerEvent('tac_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							tac.ShowNotification("Necesitas una membresia para hacer ejercicio")
						end
					elseif training == true then
						tac.ShowNotification("Debes descansar...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presiona ~INPUT_CONTEXT~ para hacer ~g~pull-ups')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then
							...
							--TriggerServerEvent('tac_gym:trainChins') ## COMING SOON...
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Presiona ~INPUT_CONTEXT~ para hacer ~g~pushups')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then				
							...
							--TriggerServerEvent('tac_gym:trainPushups') ## COMING SOON...
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then	
							...
							--TriggerServerEvent('tac_gym:trainYoga') ## COMING SOON...
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then	
							...
							--TriggerServerEvent('tac_gym:trainSitups') ## COMING SOON...
...
function CheckTraining()
	if resting == true then
		tac.ShowNotification("You are resting...")
	...
	if resting == false then
		tac.ShowNotification("You can now exercise again...")
	end
end
...
function OpenGymMenu()
    ...
    tac.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            title    = 'Gym',
            elements = {
				{label = 'Shop', value = 'Tienda'},
				{label = 'Business Hours', value = 'Horarios'},
				{label = 'Membership', value = 'Membresia'},
            }
        },
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				tac.ShowNotification("Estamos abiertos las 24hs del dia. Bienvenido!")
    ...
    tac.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            ...
            elements = {
				...
				{label = 'Barra energtica ($2)', value = 'sportlunch'},
        ...
        function(data, menu)
            if data.current.value == 'protein_shake' then
				TriggerServerEvent('tac_gym:buyProteinshake')
            elseif data.current.value == 'water' then
				TriggerServerEvent('tac_gym:buyWater')
            elseif data.current.value == 'sportlunch' then
				TriggerServerEvent('tac_gym:buySportlunch')
            elseif data.current.value == 'powerade' then
				TriggerServerEvent('tac_gym:buyPowerade')
            elseif data.current.value == 'bandage' then
				TriggerServerEvent('tac_gym:buyBandage')
            end
        end,
        function(data, menu)
            menu.close()
    ...
    tac.UI.Menu.Open(
        ...
        function(data, menu)
            if data.current.value == 'membership' then
				TriggerServerEvent('tac_gym:buyMembership')
    ...
    tac.UI.Menu.Open(
        ...
        function(data, menu)
            if data.current.value == 'bmx' then
				TriggerServerEvent('tac_gym:hireBmx')
				TriggerEvent('tac:spawnVehicle', "bmx")
				
				tac.UI.Menu.CloseAll()
            elseif data.current.value == 'cruiser' then
				TriggerServerEvent('tac_gym:hireCruiser')
				TriggerEvent('tac:spawnVehicle', "cruiser")
				
				tac.UI.Menu.CloseAll()
            elseif data.current.value == 'fixter' then
				TriggerServerEvent('tac_gym:hireFixter')
				TriggerEvent('tac:spawnVehicle', "fixter")
				
				tac.UI.Menu.CloseAll()
            elseif data.current.value == 'scorcher' then
				TriggerServerEvent('tac_gym:hireScorcher')
				TriggerEvent('tac:spawnVehicle', "scorcher")
