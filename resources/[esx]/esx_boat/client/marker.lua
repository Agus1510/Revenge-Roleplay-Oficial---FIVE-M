			...
			if IsControlJustReleased(0, 38) then
				if CurrentAction == 'boat_shop' then
					if not Config.LicenseEnable then
						...
						tac.TriggerServerCallback('tac_license:checkLicense', function(hasBoatLicense)
...
AddEventHandler('tac_boat:hasEnteredMarker', function(zone, zoneNum)
...
AddEventHandler('tac_boat:hasExitedMarker', function()
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastZone ~= currentZone or LastZoneNum ~= currentZoneNum)) then
			...
			then
				TriggerEvent('tac_boat:hasExitedMarker', LastZone)
				hasExited = true
			end
			...
			TriggerEvent('tac_boat:hasEnteredMarker', currentZone, currentZoneNum)
		end

		if not hasExited and not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_boat:hasExitedMarker')
		end
