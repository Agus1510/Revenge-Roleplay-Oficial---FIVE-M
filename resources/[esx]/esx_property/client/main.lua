...
AddEventHandler('tac:playerLoaded', function(xPlayer)
	tac.TriggerServerCallback('tac_property:getProperties', function(properties)
		Config.Properties = properties
		CreateBlips()
	end)

	tac.TriggerServerCallback('tac_property:getOwnedProperties', function(ownedProperties)
...
RegisterNetEvent('tac_property:sendProperties')
AddEventHandler('tac_property:sendProperties', function(properties)
	Config.Properties = properties
	CreateBlips()
...
function EnterProperty(name, owner)
	...
	TriggerServerEvent('tac_property:saveLastProperty', name)
...
function ExitProperty(name)
	...
	TriggerServerEvent('tac_property:deleteLastProperty')
...
function OpenPropertyMenu(property)
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'property', {
		...
		if data.current.value == 'enter' then
			TriggerEvent('instance:create', 'property', {property = property.name, owner = tac.GetPlayerData().identifier})
		elseif data.current.value == 'leave' then
			TriggerServerEvent('tac_property:removeOwnedProperty', property.name)
		elseif data.current.value == 'buy' then
			TriggerServerEvent('tac_property:buyProperty', property.name)
		elseif data.current.value == 'rent' then
			TriggerServerEvent('tac_property:rentProperty', property.name)
...
function OpenGatewayMenu(property)
	if Config.EnablePlayerManagement then
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway', {
			title    = property.name,
			align    = 'top-left',
			elements = {
...
function OpenGatewayOwnedPropertiesMenu(property)
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties', {
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_owned_properties_actions', {
			...
			if data2.current.value == 'enter' then
				TriggerEvent('instance:create', 'property', {property = data.current.value, owner = tac.GetPlayerData().identifier})
				tac.UI.Menu.CloseAll()
			elseif data2.current.value == 'leave' then
				TriggerServerEvent('tac_property:removeOwnedProperty', data.current.value)
			end
...
function OpenGatewayAvailablePropertiesMenu(property)
	...
	for i=1, #gatewayProperties, 1 do
		if not PropertyIsOwned(gatewayProperties[i]) then
			table.insert(elements, {
				label = gatewayProperties[i].label .. ' $' .. tac.Math.GroupDigits(gatewayProperties[i].price),
				value = gatewayProperties[i].name,
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties', {
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'gateway_available_properties_actions', {
			...
			if data2.current.value == 'buy' then
				TriggerServerEvent('tac_property:buyProperty', data.current.value)
			elseif data2.current.value == 'rent' then
				TriggerServerEvent('tac_property:rentProperty', data.current.value)
...
function OpenRoomMenu(property, owner)
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		title    = property.label,
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		...
		if data.current.value == 'invite_player' then
			...
			local playersInArea = tac.Game.GetPlayersInArea(entering, 10.0)
			local elements      = {}
			...
			tac.UI.Menu.Open('default', GetCurrentResourceName(), 'room_invite', {
				...
				tac.ShowNotification(_U('you_invited', GetPlayerName(data2.current.value)))
			end, function(data2, menu2)
				menu2.close()
			end)
		...
		elseif data.current.value == 'player_dressing' then
			...
			tac.TriggerServerCallback('tac_property:getPlayerDressing', function(dressing)
				...
				tac.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
		...
		elseif data.current.value == 'remove_cloth' then
			...
			tac.TriggerServerCallback('tac_property:getPlayerDressing', function(dressing)
				...
				tac.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					...
					tac.ShowNotification(_U('removed_cloth'))
				end, function(data2, menu2)
					menu2.close()
				end)
			end)
...
function OpenRoomInventoryMenu(property, owner)
	tac.TriggerServerCallback('tac_property:getPropertyInventory', function(inventory)
		local elements = {}

		if inventory.blackMoney > 0 then
			table.insert(elements, {
				label = _U('dirty_money', tac.Math.GroupDigits(inventory.blackMoney)),
				type = 'item_account',
				value = 'black_money'
			})
		end
		...
		for i=1, #inventory.weapons, 1 do
			...
			table.insert(elements, {
				label = tac.GetWeaponLabel(weapon.name) .. ' [' .. weapon.ammo .. ']',
				type  = 'item_weapon',
				value = weapon.name,
				ammo  = weapon.ammo
			})
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
			...
			if data.current.type == 'item_weapon' then
				...
				TriggerServerEvent('tac_property:getItem', owner, data.current.type, data.current.value, data.current.ammo)
				tac.SetTimeout(300, function()
					OpenRoomInventoryMenu(property, owner)
				end)
			else
				tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
					title = _U('amount')
				}, function(data2, menu)
					...
					if quantity == nil then
						...
						TriggerServerEvent('tac_property:getItem', owner, data.current.type, data.current.value, quantity)
						tac.SetTimeout(300, function()
							OpenRoomInventoryMenu(property, owner)
						end)
					end
...
function OpenPlayerInventoryMenu(property, owner)
	tac.TriggerServerCallback('tac_property:getPlayerInventory', function(inventory)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
			title    = property.label .. ' - ' .. _U('inventory'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			...
			if data.current.type == 'item_weapon' then
				menu.close()
				TriggerServerEvent('tac_property:putItem', owner, data.current.type, data.current.value, data.current.ammo)

				tac.SetTimeout(300, function()
					OpenPlayerInventoryMenu(property, owner)
				end)
			else
				tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
					title = _U('amount')
				}, function(data2, menu2)
					local quantity = tonumber(data2.value)
					...
					if quantity == nil then
						...
						TriggerServerEvent('tac_property:putItem', owner, data.current.type, data.current.value, tonumber(data2.value))
						tac.SetTimeout(300, function()
							OpenPlayerInventoryMenu(property, owner)
						end)
					end
...
AddEventHandler('playerSpawned', function()
	if firstSpawn then
		Citizen.CreateThread(function()
			while not tac.IsPlayerLoaded() do
				Citizen.Wait(0)
			end

			tac.TriggerServerCallback('tac_property:getLastProperty', function(propertyName)
				if propertyName then
					if propertyName ~= '' then
						...
						TriggerEvent('instance:create', 'property', {property = propertyName, owner = tac.GetPlayerData().identifier})
					end
				end
			end)
		end)
...
AddEventHandler('tac_property:getProperties', function(cb)
	cb(GetProperties())
end)

AddEventHandler('tac_property:getProperty', function(name, cb)
	cb(GetProperty(name))
end)

AddEventHandler('tac_property:getGateway', function(property, cb)
	cb(GetGateway(property))
end)

RegisterNetEvent('tac_property:setPropertyOwned')
AddEventHandler('tac_property:setPropertyOwned', function(name, owned)
...
AddEventHandler('tac_property:hasEnteredMarker', function(name, part)
	local property = GetProperty(name)
...
AddEventHandler('tac_property:hasExitedMarker', function(name, part)
	tac.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker or (isInMarker and (LastProperty ~= currentProperty or LastPart ~= currentPart) ) then
			...
			TriggerEvent('tac_property:hasEnteredMarker', currentProperty, currentPart)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('tac_property:hasExitedMarker', LastProperty, LastPart)
		end
