...
function LeaveInstance()
	if instance.host then
		if #instance.players > 1 then
			tac.ShowNotification(_U('left_instance'))
		end
...
AddEventHandler('instance:onPlayerEntered', function(_instance, player)
	...
	tac.ShowNotification(_('entered_into', playerName))
end)
...
AddEventHandler('instance:onPlayerLeft', function(_instance, player)
	...
	tac.ShowNotification(_('left_out', playerName))
end)
...
AddEventHandler('instance:onInvite', function(_instance, type, data)
	...
	Citizen.CreateThread(function()
		...
		if instanceInvite then
			tac.ShowNotification(_U('invite_expired'))
			instanceInvite = nil
		end
	end)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if instanceInvite then
			tac.ShowHelpNotification(_U('press_to_enter'))

			if IsControlJustReleased(0, 38) then
				EnterInstance(instanceInvite)
				tac.ShowNotification(_U('entered_instance'))
				instanceInvite = nil
			end
		else
			Citizen.Wait(500)
