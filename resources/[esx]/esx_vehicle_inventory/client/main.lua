...
AddEventHandler('tac:playerLoaded', function(xPlayer)
  	PlayerData = xPlayer
    TriggerServerEvent("tac_truck_inventory:getOwnedVehicule")
end)

RegisterNetEvent('tac_truck_inventory:setOwnedVehicule')
AddEventHandler('tac_truck_inventory:setOwnedVehicule', function(vehicle)
      ...
      if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
            ...
            if tac.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
              ...
              if locked == 1 or class == 15 or class == 16 or class == 14 then
	              ...
	              TriggerServerEvent("tac_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
	            else
	          	   tac.ShowNotification('El baul esta cerrado')
              end
            end
        else
        	tac.ShowNotification('Ningún ~ r ~ vehículo ~ w ~ cercano')
          end
      lastOpen = true
...
RegisterNetEvent('tac_truck_inventory:getInventoryLoaded')
AddEventHandler('tac_truck_inventory:getInventoryLoaded', function(inventory,weight)
	local elements = {}
  print(weight)
	...
	tac.UI.Menu.Open(
	  ...
	  function(data, menu)
	  	if data.current.value == 'deposit' then
			...
			tac.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_player', {
			    title    = 'Contenido del ivnentario',
			    align    = 'bottom-right',
			    elements = elem,
...
--[[
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
              ...
              --  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
  				      ...
  				      TriggerServerEvent('tac_truck_inventory:addInventoryItem', GetVehicleClass(closecar), GetDisplayNameFromVehicleModel(GetEntityModel(closecar)), GetVehicleNumberPlateText(vehFront), data3.current.value, quantity, data3.current.name,ownedV)
                tac.ShowNotification('Peso : ~g~'.. Kgweight .. ' Kg / '..MaxVh..' Kg')
              else
                tac.ShowNotification('Has alcanzado el limite de ~r~ '..MaxVh..' Kg')
              end
				    else
			      		tac.ShowNotification('~r~ Cantidad invalida')
				    end
          ...
          --fin test
            ...
            if not max then
              --  VehicleMaxSpeed(vehFront,poid,Config.VehicleLimit[GetVehicleClass(vehFront)])
               TriggerServerEvent('tac_truck_inventory:removeInventoryItem', GetVehicleNumberPlateText(vehFront), data.current.value, quantity)

            else
              tac.ShowNotification('~r~ Pusiste demasaido')
            end
			    else
			      tac.ShowNotification('~r~ Cantidad invalida')
			    end
