Citizen.CreateThread(function()
	...
	local openMenu = function(namespace, name, data)
		...
		local timeoutId = tac.SetTimeout(200, function()
	...
	tac.UI.Menu.RegisterType(MenuType, openMenu, closeMenu)

	RegisterNUICallback('menu_submit', function(data, cb)
		local menu = tac.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
		local post = true
		...
		if menu.submit ~= nil then
			...
			if tonumber(data.value) ~= nil then
				...
				-- Round float values
				data.value = tac.Math.Round(tonumber(data.value))
			...
			data.value = tac.Math.Trim(data.value)

			-- Don't post if the value is negative or if it's 0
			if post then
				menu.submit(data, menu)
			else
				tac.ShowNotification('That input is invalid!')
			end
		end
