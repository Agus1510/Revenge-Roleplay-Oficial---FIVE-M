...
RegisterNetEvent('tac_repairkit:onUse')
AddEventHandler('tac_repairkit:onUse', function()
	local playerPed		= GetPlayerPed(-1)
	local coords		= GetEntityCoords(playerPed)

	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		local vehicle = nil
			tac.ShowNotification(_U('not_near_engine'))
		else
		...
		then
			...
			Citizen.CreateThread(function()
				...
				if CurrentAction ~= nil then
					...
					TriggerServerEvent('tac_repairkit:removeKit')
				...
				if explosionchance == 1 and Config.ExplosionOnFailedRepair == true then
					...
					tac.ShowNotification(_U('car_exploding'))
					ClearPedTasksImmediately(playerPed)
					SetVehicleEngineHealth(vehicle, 0)
					Wait(4000)
					SetVehicleTimedExplosion(vehicle)
				else
					tac.ShowNotification(_U('finished_repair'))
				end
		...
		Citizen.CreateThread(function()
			...
			if IsControlJustPressed(0, Keys["X"]) and isReparing == true then
				...
				tac.ShowNotification(_U('aborted_repair'))
				isReparing = not isReparing
				CurrentAction = nil
...
AddEventHandler('tyrekit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		local vehicle = nil
			tac.ShowNotification(_U('not_near_tyre'))
		else
			tac.ShowNotification(_U('no_vehicle_nearby'))
		end	
		...
		then
			...
			Citizen.CreateThread(function()
				...
				if CurrentAction ~= nil then
					...
					TriggerServerEvent('tac_repairkit:removeTyreKit')
					tac.ShowNotification(_U('finished_tyre_repair'))
				if isReparing == true then
					isReparing = not isReparing
				end
		...
		Citizen.CreateThread(function()
			...
			if IsControlJustPressed(0, Keys["X"]) and isReparing == true then
				...
				tac.ShowNotification(_U('aborted_tyre_repair'))
				isReparing = not isReparing
				CurrentAction = nil
