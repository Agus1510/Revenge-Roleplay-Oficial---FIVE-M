...
tac.RegisterServerCallback("tac-qalle-sellvehicles:retrieveVehicles", function(source, cb)
	local src = source
	local identifier = tac.GetPlayerFromId(src)["identifier"]
...
tac.RegisterServerCallback("tac-qalle-sellvehicles:isVehicleValid", function(source, cb, vehicleProps, price)
	local src = source
	...
	RetrievePlayerVehicles(xPlayer.identifier, function(ownedVehicles)
		...
		for id, v in pairs(ownedVehicles) do
			...
			if Trim(plate) == Trim(v.plate) and #Config.VehiclePositions ~= VehiclesForSale then
                ...
                TriggerClientEvent("tac-qalle-sellvehicles:refreshVehicles", -1)
...
tac.RegisterServerCallback("tac-qalle-sellvehicles:buyVehicle", function(source, cb, vehProps, price)
	local src = source
	local xPlayer = tac.GetPlayerFromId(src)
	...
	if xPlayer.getAccount("bank")["money"] >= price or price == 0 then
		...
		MySQL.Async.fetchAll('SELECT seller FROM vehicles_for_sale WHERE vehicleProps LIKE "%' .. plate .. '%"', {}, function(result)
			if result[1] ~= nil and result[1]["seller"] ~= nil then
				...
				print("Algo salio mal ,no hay ningun auto.")
			end
		end)
...
function UpdateCash(identifier, cash)
	local xPlayer = tac.GetPlayerFromIdentifier(identifier)

	if xPlayer ~= nil then
		xPlayer.addAccountMoney("bank", cash)

		TriggerClientEvent("tac:showNotification", xPlayer.source, "Alguien compro tu vehiculo y pago $" .. cash)
	else
