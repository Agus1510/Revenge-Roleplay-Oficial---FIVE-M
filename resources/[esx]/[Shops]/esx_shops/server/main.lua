	...
	MySQL.Async.fetchAll('SELECT * FROM shops LEFT JOIN items ON items.name = shops.item', {}, function(shopResult)
		for i=1, #shopResult, 1 do
			if shopResult[i].name then
				...
				print(('tac_shops: invalid item "%s" found!'):format(shopResult[i].item))
			end
		end
	end)
end)
...
--tac.RegisterServerCallback('tac_shops:requestDBItems', function(source, cb)
	--cb(ShopItems)
--end)

tac.RegisterServerCallback('tac_shops:requestDBItems', function(source, cb)
	if not hasSqlRun then
		TriggerClientEvent('tac:showNotification', source, 'The shop database has not been loaded yet, try again in a few moments.')
	end
	
	cb(ShopItems)
end)

RegisterServerEvent('tac_shops:buyItem')
AddEventHandler('tac_shops:buyItem', function(itemName, amount, zone)
	...
	if amount < 0 then
		print('tac_shops: ' .. xPlayer.identifier .. ' attempted to exploit the shop!')
		return
	end
	...
	if xPlayer.getMoney() >= price then
		...
		if xPlayer.canCarryItem(itemName, amount) then
			...
			xPlayer.showNotification(_U('bought', amount, itemLabel, tac.Math.GroupDigits(price)))
		else
			xPlayer.showNotification(_U('player_cannot_hold'))			
		end
	else
		local missingMoney = price - xPlayer.getMoney()
		xPlayer.showNotification(_U('not_enough', tac.Math.GroupDigits(missingMoney)))
	end
end)
