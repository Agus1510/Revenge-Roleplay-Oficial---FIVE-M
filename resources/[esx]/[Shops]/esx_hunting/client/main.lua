					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								tac.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] Slaughter Animal', 0.4)
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										tac.ShowNotification('You need to use the knife!')
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	tac.ShowNotification('You slaughtered the animal and recieved an meat of ' ..AnimalWeight.. 'kg')

	TriggerServerEvent('tac-qalle-hunting:reward', AnimalWeight)

	DeleteEntity(AnimalId)
end

function SellItems()
	TriggerServerEvent('tac-qalle-hunting:sell')
end
...
function DrawM(hint, type, x, y, z)
	tac.Game.Utils.DrawText3D({x = x, y = y, z = z + 1.0}, hint, 0.4)
