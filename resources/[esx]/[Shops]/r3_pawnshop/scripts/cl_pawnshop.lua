			...
			if Config.EnableOpeningHours then
				if GetClockHours() >= Config.OpenHour and GetClockHours() <= Config.CloseHour - 1 then
					if not menuOpen then
						Draw3DText(Config.PawnshopLocation.x, Config.PawnshopLocation.y, Config.PawnshopLocation.z, "~g~E~w~ - Abrir la tienda de empeño")
					...
					Draw3DText(Config.PawnshopLocation.x, Config.PawnshopLocation.y, Config.PawnshopLocation.z, "Tienda cerrada,vuelve a las ~r~" .. Config.OpenHour ..":00")
				end
			else
				if not menuOpen then
					Draw3DText(Config.PawnshopLocation.x, Config.PawnshopLocation.y, Config.PawnshopLocation.z, "~g~E~w~ - Abrir la tienda de empeños")
					if IsControlJustReleased(0, 38) then
						wasOpen = true
						OpenPawnshop()
					end
...
function OpenPawnshop()
	...
	for k, v in pairs(tac.GetPlayerData().inventory) do
		local price = Config.PawnshopItems[v.name]

		if price and v.count > 0 then
			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(v.label, "$" .. tac.Math.GroupDigits(price)),
				name = v.name,
				price = price,
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'pawnshop', {
		title    = "Pawnshop",
