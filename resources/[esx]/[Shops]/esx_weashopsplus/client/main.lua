...
RegisterNetEvent('tac_weashop:loadLicenses')
AddEventHandler('tac_weashop:loadLicenses', function (licenses)
...
function OpenMainMenu(zone)
  ...
  tac.UI.Menu.Open(
    ...
    {
      title = "Categorias de armas",
	  align    = 'left',
      elements = {
        { label = "Armas", value = 'wl_1' },
      }
    },
    function (data, menu)
  ...
  tac.UI.Menu.Open(
    ...
    function (data, menu)
      if data.current.value == 'yes' then
        TriggerServerEvent('tac_weashop:buyLicense', price, wtype)
      end
      menu.close()
    end,
    function (data, menu)
...
function OpenShopMenu(rvalue, zone)
  ...
  if rvalue == "wl_1" then
    ...
    tac.UI.Menu.Open(
      ...
      function(data, menu)
        TriggerServerEvent('tac_weashop:buyItem', data.current.value, data.current.price, zone)
      end,
      function(data, menu)
...
AddEventHandler('tac_weashop:hasEnteredMarker', function(zone)
...
AddEventHandler('tac_weashop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
  while true do
    ...
    if isInMarker and not HasAlreadyEnteredMarker then
      ...
      TriggerEvent('tac_weashop:hasEnteredMarker', currentZone)
    end
    if not isInMarker and HasAlreadyEnteredMarker then
      HasAlreadyEnteredMarker = false
      TriggerEvent('tac_weashop:hasExitedMarker', LastZone)
    end
  end
end)
