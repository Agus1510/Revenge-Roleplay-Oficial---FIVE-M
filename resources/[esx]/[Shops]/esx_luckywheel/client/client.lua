...
local _wheelPos = vector3(977.47, 49.52, 73.66)
local _baseWheelPos = vector3(977.29, 49.14, 73.68)
local Keys = {
...
Citizen.CreateThread(function()
    ...
    if tac.IsPlayerLoaded() then
        ...
        Citizen.CreateThread(function()
            ...
            _wheel = CreateObject(model, 977.94, 50.22, 74.86, false, false, true)
            ...
            local vehicle = CreateVehicle(carmodel, 963.42, 48.23, 74.57, 0.0, false, false)
...
RegisterNetEvent("tac_luckywheel:doRoll")
AddEventHandler("tac_luckywheel:doRoll", function(_priceIndex) 
    _isRolling = true
...
RegisterNetEvent("tac_luckywheel:rollFinished")
AddEventHandler("tac_luckywheel:rollFinished", function() 
    _isRolling = false
end)
...
function doRoll()
    if not _isRolling then
        ...
        tac.Streaming.RequestAnimDict(lib, function()
            local _movePos = vector3(976.99, 50.09, 73.68)
            ...
            TriggerServerEvent("tac_luckywheel:getLucky")
...
Citizen.CreateThread(function()
	while true do
        ...
        if(GetDistanceBetweenCoords(coords, _wheelPos.x, _wheelPos.y, _wheelPos.z, true) < 1.5) and not _isRolling then
            tac.ShowHelpNotification("Presione E para probar su suerte en la rueda de la fortuna, valor 50,000$")
