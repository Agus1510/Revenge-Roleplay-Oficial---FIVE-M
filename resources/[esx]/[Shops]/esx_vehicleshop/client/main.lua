...
Citizen.CreateThread(function()
	...
	tac.TriggerServerCallback('tac_vehicleshop:getCategories', function(categories)
		Categories = categories
	end)

	tac.TriggerServerCallback('tac_vehicleshop:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)

	if Config.EnablePlayerManagement then
		if tac.PlayerData.job.name == 'cardealer' then
...
RegisterNetEvent('tac_vehicleshop:sendCategories')
AddEventHandler('tac_vehicleshop:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('tac_vehicleshop:sendVehicles')
AddEventHandler('tac_vehicleshop:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	tac.TriggerServerCallback('tac_vehicleshop:getCommercialVehicles', function(vehicles)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('tac_vehicleshop:returnProvider', data.current.value)
...
function OpenShopMenu()
	if #Vehicles == 0 then
		print('[tac_vehicleshop] [^3ERROR^7] No vehicles found')
		return
	end
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('[tac_vehicleshop] [^3ERROR^7] Vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					tac.TriggerServerCallback('tac_vehicleshop:buyCarDealerVehicle', function(success)
						if success then
							IsInShopMenu = false
					...
					tac.TriggerServerCallback('tac_vehicleshop:buyVehicle', function(success)
						if success then
							IsInShopMenu = false
							menu2.close()
							menu.close()
				...
				tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('tac_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			if currentDisplayVehicle then
				...
				if closestPlayer ~= -1 and closestDistance < 3 then
					...
					TriggerServerEvent('tac_vehicleshop:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps, CurrentVehicleData.model, CurrentVehicleData.name)
					...
					tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
						...
						if not amount then
							...
							if closestPlayer ~= -1 and closestDistance < 3 then
								...
								SetVehicleNumberPlateText(currentDisplayVehicle, newPlate)
								TriggerServerEvent('tac_vehicleshop:rentVehicle', model, newPlate, amount, GetPlayerServerId(closestPlayer))
								currentDisplayVehicle = nil
							else
								tac.ShowNotification(_U('no_players'))
							end
...
function OpenRentedVehiclesMenu()
	tac.TriggerServerCallback('tac_vehicleshop:getRentedVehicles', function(vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('tac_society:openBossMenu', 'cardealer', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			tac.TriggerServerCallback('tac_vehicleshop:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	tac.TriggerServerCallback('tac_vehicleshop:getStockItems', function(items)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('tac_vehicleshop:getStockItem', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	tac.TriggerServerCallback('tac_vehicleshop:getPlayerInventory', function(inventory)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('tac_vehicleshop:putStockItems', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('tac_vehicleshop:hasEnteredMarker', function(zone)
	if zone == 'ShopEntering' then

		if Config.EnablePlayerManagement then
			if tac.PlayerData.job ~= nil and tac.PlayerData.job.name == 'cardealer' then
				CurrentAction     = 'reseller_menu'
				CurrentActionMsg  = _U('shop_menu')
				CurrentActionData = {}
			end
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and tac.PlayerData.job ~= nil and tac.PlayerData.job.name == 'cardealer' and tac.PlayerData.job.grade_name == 'boss' then
		CurrentAction     = 'boss_actions_menu'
		CurrentActionMsg  = _U('shop_menu')
		CurrentActionData = {}
	end
...
AddEventHandler('tac_vehicleshop:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('tac_vehicleshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_vehicleshop:hasExitedMarker', LastZone)
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'give_back_vehicle' then
					tac.TriggerServerCallback('tac_vehicleshop:giveBackVehicle', function(isRentedVehicle)
				...
				elseif CurrentAction == 'resell_vehicle' then
					tac.TriggerServerCallback('tac_vehicleshop:resellVehicle', function(vehicleSold)
						if vehicleSold then
							tac.Game.DeleteVehicle(CurrentActionData.vehicle)
