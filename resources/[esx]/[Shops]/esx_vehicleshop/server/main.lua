...
TriggerEvent('tac_phone:registerNumber', 'cardealer', _U('dealer_customers'), false, false)
TriggerEvent('tac_society:registerSociety', 'cardealer', _U('car_dealer'), 'society_cardealer', 'society_cardealer', 'society_cardealer', {type = 'private'})
...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('[tac_vehicleshop] [^3WARNING^7] Plate character count reached, %s/8 characters!'):format(char))
	end
end)
...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM vehicle_categories', {}, function(_categories)
		...
		MySQL.Async.fetchAll('SELECT * FROM vehicles', {}, function(_vehicles)
			...
			TriggerClientEvent('tac_vehicleshop:sendCategories', -1, categories)
			TriggerClientEvent('tac_vehicleshop:sendVehicles', -1, vehicles)
		end)
	end)
end)
...
RegisterNetEvent('tac_vehicleshop:setVehicleOwnedPlayerId')
AddEventHandler('tac_vehicleshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
...
tac.RegisterServerCallback('tac_vehicleshop:getSoldVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT client, model, plate, soldby, date FROM vehicle_sold', {}, function(result)
		cb(result)
	end)
end)

RegisterNetEvent('tac_vehicleshop:rentVehicle')
AddEventHandler('tac_vehicleshop:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = tac.GetPlayerFromId(source), tac.GetPlayerFromId(playerId)
...
RegisterNetEvent('tac_vehicleshop:getStockItem')
AddEventHandler('tac_vehicleshop:getStockItem', function(itemName, count)
	local _source = source
	local xPlayer = tac.GetPlayerFromId(_source)

	TriggerEvent('tac_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
...
RegisterNetEvent('tac_vehicleshop:putStockItems')
AddEventHandler('tac_vehicleshop:putStockItems', function(itemName, count)
	local _source = source
	local xPlayer = tac.GetPlayerFromId(_source)
...
tac.RegisterServerCallback('tac_vehicleshop:getCategories', function(source, cb)
	cb(categories)
end)

tac.RegisterServerCallback('tac_vehicleshop:getVehicles', function(source, cb)
	cb(vehicles)
end)

tac.RegisterServerCallback('tac_vehicleshop:buyVehicle', function(source, cb, model, plate)
...
tac.RegisterServerCallback('tac_vehicleshop:getCommercialVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT price, vehicle FROM cardealer_vehicles ORDER BY vehicle ASC', {}, function(result)
		cb(result)
	end)
end)

tac.RegisterServerCallback('tac_vehicleshop:buyCarDealerVehicle', function(source, cb, model)
	local xPlayer = tac.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'cardealer' then
		...
		if modelPrice then
			TriggerEvent('tac_addonaccount:getSharedAccount', 'society_cardealer', function(account)
...
RegisterNetEvent('tac_vehicleshop:returnProvider')
AddEventHandler('tac_vehicleshop:returnProvider', function(vehicleModel)
	local xPlayer = tac.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'cardealer' then
		MySQL.Async.fetchAll('SELECT id, price FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			...
			if result[1] then
				...
				print(('[tac_vehicleshop] [^3WARNING^7] %s attempted selling an invalid vehicle!'):format(xPlayer.identifier))
			end
		end)
	end
end)

tac.RegisterServerCallback('tac_vehicleshop:getRentedVehicles', function(source, cb)
...
tac.RegisterServerCallback('tac_vehicleshop:giveBackVehicle', function(source, cb, plate)
...
tac.RegisterServerCallback('tac_vehicleshop:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'cardealer' then
		...
		if not resellPrice then
			print(('[tac_vehicleshop] [^3WARNING^7] %s attempted to sell an unknown vehicle!'):format(xPlayer.identifier))
			cb(false)
		else
						...
						if result[1] then -- does the owner match?
							...
							if vehicle.model == model then
								if vehicle.plate == plate then
									...
									print(('[tac_vehicleshop] [^3WARNING^7] %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
									cb(false)
								end
							else
								print(('[tac_vehicleshop] [^3WARNING^7] %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						end
					end)
...
tac.RegisterServerCallback('tac_vehicleshop:getStockItems', function(source, cb)
	TriggerEvent('tac_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)

tac.RegisterServerCallback('tac_vehicleshop:getPlayerInventory', function(source, cb)
	local xPlayer = tac.GetPlayerFromId(source)
...
tac.RegisterServerCallback('tac_vehicleshop:isPlateTaken', function(source, cb, plate)
...
tac.RegisterServerCallback('tac_vehicleshop:retrieveJobVehicles', function(source, cb, type)
	local xPlayer = tac.GetPlayerFromId(source)
...
RegisterNetEvent('tac_vehicleshop:setJobVehicleState')
AddEventHandler('tac_vehicleshop:setJobVehicleState', function(plate, state)
	local xPlayer = tac.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[tac_vehicleshop] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
...
function PayRent(d, h, m)
	...
	print('[tac_vehicleshop] [^2INFO^7] Paying rent cron job started')
	...
	MySQL.Async.fetchAll('SELECT owner, rent_price FROM rented_vehicles', {}, function(result)
		...
		print(('[tac_vehicleshop] [^2INFO^7] Paying rent cron job took %s seconds'):format(elapsedTime))
	end)
end
