...
Citizen.CreateThread(function()
    while isLoading == true do 
        ...
        if tac.IsPlayerLoaded(PlayerId) and playerData.job.name == "police" then 
...
RegisterNUICallback('searchPlate', function(data, cb)
    local vehicle = tac.Game.GetVehicles()
    local miss = 0

    for i=1, #vehicle, 1 do 
        local vehicleProps = tac.Game.GetVehicleProperties(vehicle[i])
...
function createVehicleTracker(vehicle, plate) 
    triggerTimer(plate)
        ...
        tac.ShowNotification(_U('tracker_activated') .. plate)
        Citizen.CreateThread(function()
            while time_out[plate] == false do
                ...
                if DoesEntityExist(vehicle) then 
                    ...
                    tac.ShowNotification(_U('tracker_lost') .. plate)
                end
            end 
            RemoveBlip(blips_pos[plate])
            time_out[plate] = time_out[nil]
