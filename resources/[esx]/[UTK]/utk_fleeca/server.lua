        ...
        if item >= 1 then
            if not UTK.Banks[bank].onaction == true then
                if (os.time() - UTK.cooldown) > UTK.Banks[bank].lastrobbed then
                    ...
                    TriggerClientEvent("utk_fh:outcome", _source, false, "Este banco ya fue robado, tienes que esperar para poder robar "..math.floor((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)) / 60)..":"..math.fmod((UTK.cooldown - (os.time() - UTK.Banks[bank].lastrobbed)), 60))
                end
            else
                TriggerClientEvent("utk_fh:outcome", _source, false, "Este banco esta siendo robado.")
            end
        else
            TriggerClientEvent("utk_fh:outcome", _source, false, "No tienes la tarjeta de acceso.")
        end
    else
        TriggerClientEvent("utk_fh:outcome", _source, false, "No hay suficientes policias en al ciudad.")
    end
end)
...
tac.RegisterServerCallback("utk_fh:getBanks", function(source, cb)
    cb(UTK.Banks, Doors)
end)

tac.RegisterServerCallback("utk_fh:checkSecond", function(source, cb)
    local xPlayer = tac.GetPlayerFromId(source)
