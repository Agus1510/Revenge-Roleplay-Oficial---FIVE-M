            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Agarrar el dinero", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Agarrar el dinero", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Agarrar el dinero", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    local PlayerData = tac.GetPlayerData()
    local blip = nil
    ...
    if PlayerData.job.name == "police" then
        exports["mythic_notify"]:DoHudText("informe", "La alarma del banco se activo!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
            ...
            if PlayerData.job.name == "police" and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Desbloquear puerta", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Bloquear puerta", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    exports["mythic_notify"]:DoHudText("error", "La bobeda se cerrar en 60 segundos!")
    Citizen.Wait(30000)
    exports["mythic_notify"]:DoHudText("error", "Bobeda cerrandose!")
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports['progressBars']:startUI(2000, "Usando tarjeta maliciosa")
    Citizen.Wait(1500)
    ...
    exports["mythic_notify"]:DoHudText("success", "Hackeo completo!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
    TriggerServerEvent("utk_fh:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    exports["mythic_notify"]:DoHudText("error", "Tienes 2 minutos hasta que se active el sistema de seguridad.")
...
function SecondDoor(data, key)
    ...
    exports["mythic_notify"]:DoHudText("success", "Acceso completo!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
...
function SpawnTrolleys(data, name)
    ...
    local players, nearbyPlayer = tac.Game.GetPlayersInArea(GetEntityCoords(PlayerPedId()), 20.0)
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "[~r~E~w~] Usar tarjeta de identificacion", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    tac.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        if result then
                            done = true
                            return SecondDoor(UTK.Banks[currentname], currentname)
                        elseif not result then
                            exports["mythic_notify"]:DoHudText("error", "No tienes una tarjeta .")
                        end
                    end)
                end
            end
...
Citizen.CreateThread(function()
    ...
    tac.TriggerServerCallback("utk_fh:getBanks", function(bank, door)
        UTK.Banks = bank
        Doors = door
    end)
    --Citizen.Wait(1000)
