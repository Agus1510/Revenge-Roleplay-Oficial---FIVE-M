...
function UpdateCredit(xPlayer, amount, style, sourceid, amount2, receiver, cash)
    ...
    if Config.logs then
        if style ~= nil then
            local yPlayer = tac.GetPlayerFromId(sourceid)
            if style == 1 then
                TriggerClientEvent("utk_c:creditfeed", receiver, amount)
                TriggerClientEvent('chat:addMessage', receiver, { args = { '^1Creditos', "^* Recibiste ^2"..amount2.."^0 creditos. nuevo monto: ^2"..math.floor(amount)}})
                TriggerClientEvent('chat:addMessage', sourceid, { args = { '^1Creditos', " "..yPlayer.name.."^2^* [ID: "..receiver..']^0 Gastados ^2'..amount2.."^0 creditos. Nuevo monto: ^2"..math.floor(amount)}})
                DiscordLog("Command-Adding", "(Command user: **"..yPlayer.name.."**) Agregados: **"..amount2.."** | Nuevo monto: **"..amount.."** \n (Steam name: **"..xPlayer.name.."**, Hex: **"..xPlayer.identifier.."**)", 15844367)
                return
            elseif style == 2 then
                TriggerClientEvent("utk_c:creditfeed", receiver, amount)
                TriggerClientEvent('chat:addMessage', receiver, { args = { '^1Creditos', "^* ^1"..amount2.."^0 Tu cantidad de creditos bajo. Nuevo monto: ^1"..math.floor(amount)}})
                TriggerClientEvent('chat:addMessage', sourceid, { args = { '^1Creditos', " "..yPlayer.name.."^2^* [ID: "..receiver..']^0 Bajó ^2'..amount2.."^0 creditos. Nuevo monto: ^2"..math.floor(amount)}})
                DiscordLog("Command-Removing", "(Command user: **"..yPlayer.name.."**) Monto removido: **"..amount2.."** | Nuevo monto: **"..amount.."** \n (Steam name: **"..xPlayer.name.."**, Hex: **"..xPlayer.identifier.."**)", 15844367)
                return
            elseif style == "exchange" then
                TriggerClientEvent("utk_c:creditfeed", sourceid, amount)
                DiscordLog("Exchange", "**"..amount2.."** Cambiaste los creditos por **"..tac.Math.GroupDigits(cash).."$**. | Nuevo monto: **"..amount.."** \n (Steam name: **"..xPlayer.name.."**, Hex: **"..xPlayer.identifier.."**)", 15844367)
                return
            end
        end
        TriggerClientEvent("utk_c:creditfeed", sourceid, amount)
...
AddEventHandler("utk_c:checkplayer", function()
    ...
    while xPlayer == nil do
        xPlayer = tac.GetPlayerFromId(_source)
        Citizen.Wait(1000)
    end
    for i = 1, #result, 1 do
...
AddEventHandler("utk_c:getcredit", function(output, id)
    ...
    local xPlayer, result = tac.GetPlayerFromId(id)
...
AddEventHandler("utk_c:addcredit", function(id, amount)
    local xPlayer = tac.GetPlayerFromId(id)
    ...
    TriggerClientEvent("utk_c:creditfeed", id, amount + oldamount)
end)
...
tac.RegisterServerCallback("utk_c:getcredit", function(source, cb)
    local _source = source
    local xPlayer = tac.GetPlayerFromId(_source)
    local amount
    local result
...
tac.RegisterServerCallback("utk_c:checkitem", function(source, cb, item)
    local xPlayer = tac.GetPlayerFromId(source)
...
tac.RegisterServerCallback('utk_c:platecheck', function(source, cb, plate)
...
tac.RegisterServerCallback("utk_c:getVehList", function(source, cb)
    local xPlayer = tac.GetPlayerFromId(source)
            ...
            if tonumber(args[2]) ~= nil then
                if math.floor(tonumber(args[2])) == tonumber(args[2]) or math.ceil(tonumber(args[2])) == tonumber(args[2]) then
                    if GetPlayerName(tonumber(args[1])) ~= nil then
                        local xPlayer = tac.GetPlayerFromId(tonumber(args[1]))
