...
local mincash = 30000 -- minimum amount of cash a pile holds
local maxcash = 100000 -- maximum amount of cash a pile can hold
...
tac.RegisterServerCallback("utk_oh:GetData", function(source, cb)
    cb(info)
end)
tac.RegisterServerCallback("utk_oh:GetDoors", function(source, cb)
    cb(PoliceDoors)
end)
tac.RegisterServerCallback("utk_oh:startevent", function(source, cb, method)
    local xPlayers = tac.GetPlayers()
    local copcount = 0
    local yPlayer = tac.GetPlayerFromId(source)
            ...
            if copcount >= mincops then
                ...
                elseif method == 2 then
                    ...
                    if item >= 1 then
                        ...
                        cb("No tienes ganzuas.")
                    end
                end
            else
                cb("Debe haber al menos "..mincops.." policias en la ciudad.")
            end
        else
            cb(math.floor((cooldown - (os.time() - lastrobbed)) / 60)..":"..math.fmod((cooldown - (os.time() - lastrobbed)), 60).." hasta poder robar de vuelta.")
        end
    else
        cb("El banco central esta siendo robado.")
    end
end)
tac.RegisterServerCallback("utk_oh:checkItem", function(source, cb, itemname)
...
tac.RegisterServerCallback("utk_oh:gettotalcash", function(source, cb)
    cb(totalcash)
end)
