                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "No tienes una tarjeta de identificacion.")
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        tac.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "No tienes una carga termica.")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        tac.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports['mythic_notify']:SendAlert("error", "No tienes cargas termicas.")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            tac.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports['mythic_notify']:SendAlert("error", "No tienes ganzuas.")
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        ...
        tac.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            ...
            exports["mythic_notify"]:SendAlert("success", "Robaste "..text)
        end)
    else
        exports['mythic_notify']:SendAlert("error", "Tienes 90 segundos hasta que llegue el gas toxico.")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["mythic_notify"]:SendAlert("error", "La puerta de la bobeda se cerrar automaticamente en 60 segundos!")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            exports["mythic_notify"]:SendAlert("infrom", "Pacific Standart Bank Se activaron las alarmas!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
