...
UTK = {
    ...
    alarmblip,
    text = { -- Texts
        loudstart = "[~r~E~w~] Empezar robo al banco ~g~Ruidoso~w~",
        silentstart = "[~r~E~w~] Empezar robo al banco ~g~Silencioso~w~ ",
        usecard = "[~r~E~w~] Usar tarjeta de identifiacion",
        usethermal = "[~r~E~w~] Usar Carga termica",
        usehack = "[~r~E~w~] Hackear panel de seguridad",
        uselockpick = "[~r~E~w~] Usar ganzua",
        usesearch = "[~r~E~w~] Buscar",
        lootcash= "[~r~E~w~] Agarrar carro de dinero",
        lootgold = "[~r~E~w~] Agarrar carro de oro",
        lootdia = "[~r~E~w~] Agarrar carro de diamantes",
        card = "Usando ID card",
        thermal = "Plantando carga termica",
        burning = "Derritiendo la cerradura",
        lockpick = "Abriendo cerradura",
        using = "Procesando panel",
        used = "Proceso completo.",
        stage = "completado.",
        search = "Buscando",
        hacking = "Hackeando",
        melted = "Cerradura derretida.",
        hacked = "Hackeo completado.",
        unlocked = "Puerta abierta.",
        nothing = "No encontraste nada.",
        found = "Encontraste la tarjeta de identificacion.",
        time = "Restante: "
    }
}
UTK.searchinfo = {
...
function UTK:GetInfo()
    tac.TriggerServerCallback("utk_oh:GetData", function(output)
        self.info = output
        self:HandleInfo()
    end)
end
                    ...
                    local coords = GetEntityCoords(PlayerPedId())
                        ...
                        if dst <= 6 then
                            ...
                            if dst <= 1 and IsControlJustReleased(0,38) then
                                tac.TriggerServerCallback("utk_oh:startevent", function(output)
                                    if output == true then
                                        UTK.initiator = true
                                        self.info.stage = 1
                                        self.info.style = 1
                        ...
                        while true do
                            ...
                            if not UTK.checks.hack1 then
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        tac.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                UTK.checks.hack1 = true
                                                self.currenthack = 0
                                                self:Hack()
                                            elseif not output then
                                                exports["pNotify"]:SendNotification({text = "No tienes una laptop de hacker.", type = "error", timeout = 5000})
                                            end
                                        end, "laptop_h")
                                    end
                                end
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            exports["pNotify"]:SendNotification({text = "No tienes una identificacion.", type = "error", timeout = 5000})
                                        end
                                        end, "id_card")
                                    end
                                end
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        tac.TriggerServerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                exports["pNotify"]:SendNotification({text = "No tienes una carga termica.", type = "error", timeout = 5000})
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            tac.TriggerServerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    exports["pNotify"]:SendNotification({text = "No tienes ganzuas.", type = "error", timeout = 5000})
                                                end
                                            end, "lockpick")
                                        end
                                    end
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    exports["pNotify"]:SendNotification({text = "Abrir my computer y entrar a HackConnect.exe", type = "success", timeout = 5000})
...
function UTK:IdCard()
    ...
    local panel = tac.Game.GetClosestObject(("hei_prop_hei_securitypanel"), pedco)
...
AddEventHandler("utk_oh:vault", function(method)
	local obj = tac.Game.GetClosestObject(UTK.vault.type, vector3(UTK.vault.x, UTK.vault.y, UTK.vault.z))
    local count = 0
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        exports["pNotify"]:SendNotification({text = "Tienes 90 segundos hasta que llegue el gas toxico.", type = "error", timeout = 5000})
        UTK.starttimer = true
        UTK.vaulttime = 150
        Citizen.CreateThread(function()
            while true do
        ...
        tac.TriggerServerCallback("utk_oh:gettotalcash", function(result)
            text = "$"..tac.Math.GroupDigits(result)
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        exports["pNotify"]:SendNotification({text = "La puerta de la boveda se cerrara en 60 segundos!", type = "error", timeout = 5000})
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if UTK.begingas then
                ...
                UTK.stage2break = true
                UTK.stage4break = true
                exports["pNotify"]:SendNotification({text = "Boveda cerrandose!", type = "error", timeout = 5000})
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    local player = tac.GetPlayerData()
    if player.job.name == "police" then
        if toggle == 1 then
            exports["pNotify"]:SendNotification({text = "Pacific Standart Bank se activaron las alarmas!", type = "error", timeout = 10000})
...
AddEventHandler("utk_oh:toggleDoor_c", function(door, coord, status)
    local obj = tac.Game.GetClosestObject(door, coord)
...
Citizen.CreateThread(function()
    ...
    tac.TriggerServerCallback("utk_oh:GetDoors", function(result)
        PoliceDoors = result
    end)
        ...
        for k, v in ipairs(PoliceDoors) do
            ...
            if dst <= 50 then
                if v.locked then
                    ...
                    doortext = "[~r~E~w~] Desbloquear puerta"
                    state = false
                elseif not v.locked then
                    FreezeEntityPosition(v.obj, false)
                    doortext = "[~r~E~w~] Bloquear puerta"
                    state = true
                end
                if PlayerData.job.name == "police" then
                    if dst <= 1.5 then
...
Citizen.CreateThread(function() while tac == nil do TriggerEvent('tac:getSharedObject', function(obj) tac = obj end) Citizen.Wait(1) end Citizen.Wait(10000) UTK:GetInfo() end)
