...
RegisterNetEvent('tac_atm:closeATM')
AddEventHandler('tac_atm:closeATM', function()
	SetNuiFocus(false)
	menuIsShowed = false
	SendNUIMessage({
		hideAll = true
...
RegisterNUICallback('escape', function(data, cb)
	TriggerEvent('tac_atm:closeATM')
	cb('ok')
end)

RegisterNUICallback('deposit', function(data, cb)
	TriggerServerEvent('tac_atm:deposit', data.amount)
	cb('ok')
end)

RegisterNUICallback('withdraw', function(data, cb)
	TriggerServerEvent('tac_atm:withdraw', data.amount)
	cb('ok')
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInATMMarker and not menuIsShowed then
			...
			tac.ShowHelpNotification(_U('press_e_atm'))

			if IsControlJustReleased(0, Keys['E']) and IsPedOnFoot(PlayerPedId()) then
				menuIsShowed = true
				tac.TriggerServerCallback('tac:getPlayerData', function(data)
					SendNUIMessage({
						showMenu = true,
						player = {
							money = data.money,
