...
AddEventHandler('tac_securitycam:hasEnteredMarker', function (zone)
...
AddEventHandler('tac_securitycam:hasExitedMarker', function(zone)
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('tac_securitycam:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_securitycam:hasExitedMarker', LastZone)
			CurrentAction = nil
	...
	while true do
					...
					if data.current.value == 'bankmenu' then
						...
						if bankHacked then
							if Config.pNotify then
								...
								tac.ShowNotification(_U('broken_cameras'))
							end
						else
							menu.close()
							bankcamera = true
							...
							menuopen = false
							TriggerEvent('tac_securitycam:freeze', true)
						end
		...
		if createdCamera ~= 0 then
			...
			if IsControlJustPressed(0, Keys["BACKSPACE"]) then
				...
				TriggerEvent('tac_securitycam:freeze', false)
			end
...
function StopHideHUD()
	Citizen.CreateThread(function()
		while not blockbuttons do
			...
			TriggerEvent('tac_status:setDisplay', 1.0)
		end
	end)
end
...
function mycbpolice(success, timeRemaining)
	if success then
		...
		TriggerServerEvent('tac_securitycam:setPoliceHackedState', true)
		...
		if Config.pNotify then
			...
			tac.ShowNotification(_U('hacking_succeed'))
		end
	else
		TriggerEvent('mhacking:hide')
		if Config.pNotify then
			...
			tac.ShowNotification(_U('hacking_failed'))
		end
		...
		TriggerServerEvent('tac_phone:send', 'police', 'POLICE ALARM: SOMEONE TRIED TO HACK OUR COMPUTERS. COME TO THE STATION FAST.', false, false)
	end
end
...
function mycbbank(success, timeRemaining)
	if success then
		TriggerEvent('mhacking:hide')
		TriggerServerEvent('tac_securitycam:setBankHackedState', true)
		if Config.pNotify then
			TriggerEvent("pNotify:SendNotification", {
				text = _U('hacking_succeed'),
				type = "success",
		...
		Citizen.Wait(500)
		PlaySoundFrontend(-1, "TextHit", "WastedSounds", 1)
		TriggerServerEvent('tac_phone:send', 'police', 'BANK: HELP! SOMEONE TRIED TO HACK MY COMPUTER! COME HERE RIGHT AWAY OR LOOK IN THE CAMERAS FAST.', false, false)
	end
end
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'hackingPolice' then
			if policeHacked then
				if Config.pNotify then
					...
					tac.ShowNotification(_U('infected_cameras'))
				end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'UnHackBank' then
			...
			if bankHacked then
				TriggerServerEvent('tac_securitycam:unhackanimserver')
				TriggerServerEvent('tac_securitycam:setBankHackedState', false)
				...
				if Config.pNotify then
					...
					tac.ShowNotification(_U('removing_viruses'))
				end
			...
			elseif not bankHacked then
				...
				if Config.pNotify then
					...
					tac.ShowNotification(_U('nothing_wrong'))
				end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'UnHackPolice' then
			if policeHacked then
				TriggerServerEvent('tac_securitycam:unhackanimserver')
				TriggerServerEvent('tac_securitycam:setPoliceHackedState', false)
...
RegisterNetEvent('tac_securitycam:freeze')
AddEventHandler('tac_securitycam:freeze', function(freeze)
...
RegisterNetEvent('tac_securitycam:unhackanim')
AddEventHandler('tac_securitycam:unhackanim', function()
...
RegisterNetEvent('tac_securitycam:setBankHackedState')
AddEventHandler('tac_securitycam:setBankHackedState', function(state)
	bankHacked = state
end)

RegisterNetEvent('tac_securitycam:setPoliceHackedState')
