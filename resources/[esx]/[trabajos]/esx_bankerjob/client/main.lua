...
function OpenBankActionsMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'bank_actions', {
		title    = _U('bank'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
			...
			tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						tac.ShowNotification(_U('no_player_nearby'))
					else
						TriggerServerEvent('tac_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_banker', 'Banque', amount)
					end
				end
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('tac_society:openBossMenu', 'banker', function (data, menu)
			menu.close()
			end, {wash = false})
		end
	end, function(data, menu)
...
function OpenCustomersMenu()
	tac.TriggerServerCallback('tac_bankerjob:getCustomers', function(customers)
		local elements = {
		...
		tac.UI.Menu.Open('list', GetCurrentResourceName(), 'customers', elements, function(data, menu)
			...
			if data.value == 'deposit' then
				...
				tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'customer_deposit_amount', {
					...
					if amount == nil then
						...
						menu2.close()
						TriggerServerEvent('tac_bankerjob:customerDeposit', customer.source, amount)
						OpenCustomersMenu()
					end
			...
			elseif data.value == 'withdraw' then
				...
				tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'customer_withdraw_amount', {
					...
					if amount == nil then
						...
						menu2.close()
						TriggerServerEvent('tac_bankerjob:customerWithdraw', customer.source, amount)
						OpenCustomersMenu()
					end
				end, function(data2, menu2)
					menu2.close()
...
AddEventHandler('tac_bankerjob:hasEnteredMarker', function (zone)
	if zone == 'BankActions' and tac.PlayerData.job and tac.PlayerData.job.name == 'banker' then
		CurrentAction     = 'bank_actions_menu'
		CurrentActionMsg  = _U('press_input_context_to_open_menu')
		CurrentActionData = {}
	end
...
AddEventHandler('tac_bankerjob:hasExitedMarker', function (zone)
	CurrentAction = nil
	tac.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if tac.PlayerData.job and tac.PlayerData.job.name == 'banker' then
			...
			if isInMarker and not hasAlreadyEnteredMarker then
				...
				TriggerEvent('tac_bankerjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and hasAlreadyEnteredMarker then
				hasAlreadyEnteredMarker = false
				TriggerEvent('tac_bankerjob:hasExitedMarker', lastZone)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and tac.PlayerData.job and tac.PlayerData.job.name == 'banker' then
