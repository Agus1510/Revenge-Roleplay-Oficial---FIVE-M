...
function FinishDelivery()
  if IsVehicleAttachedToTrailer(truck) and (GetVehiclePedIsIn(GetPlayerPed(-1), false) == truck) then
    ...
    --Pay the poor fella
    TriggerServerEvent('tac_truckerjob:pay', alldeliveries[randomdelivery].payment)
    ...
    isDelivered = 1
	  ...
	  TriggerEvent('tac:showNotification', "You have to use the trailer that was provided for you.")
  end
end
...
AddEventHandler('tac_truckerjob:hasEnteredMarker', function(zone)
  if actualZone == 'menutrucker' then
    CurrentAction     = 'trucker_menu'
    CurrentActionMsg  = 'Presiona ~INPUT_CONTEXT~ para empezar una mision'
    CurrentActionData = {zone = zone}
  elseif actualZone == 'delivered' then
    CurrentAction     = 'delivered_menu'
    CurrentActionMsg  = 'Presiona ~INPUT_CONTEXT~ para entregar'
    CurrentActionData = {zone = zone}
  elseif actualZone == 'abort' then
	  CurrentAction     = 'abort_menu'
    CurrentActionMsg  = 'Presiona ~INPUT_CONTEXT~ para cortar la entrega'
    CurrentActionData = {zone = zone}
  end
end)

AddEventHandler('tac_truckerjob:hasExitedMarker', function(zone)
	CurrentAction = nil
...
Citizen.CreateThread(function()
  while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('tac_truckerjob:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_truckerjob:hasExitedMarker', LastZone)
		end
	end
end)
