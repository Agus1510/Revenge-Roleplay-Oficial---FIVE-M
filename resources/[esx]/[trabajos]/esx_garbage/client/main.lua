...
RegisterNetEvent('tac_garbage:setbin')
AddEventHandler('tac_garbage:setbin', function(binpos, platenumber,  bags)
	if isInService then
...
RegisterNetEvent('tac_garbage:addbags')
AddEventHandler('tac_garbage:addbags', function(platenumber, bags, crewmember)
	if isInService then
...
RegisterNetEvent('tac_garbage:startpayrequest')
AddEventHandler('tac_garbage:startpayrequest', function(platenumber, amount)
	if isInService then
		if platenumb == platenumber then
			TriggerServerEvent('tac_garbage:pay', amount)
			platenumb = nil
		end
	end
end)

RegisterNetEvent('tac_garbage:removedbag')
AddEventHandler('tac_garbage:removedbag', function(platenumber)
	if isInService then
		if platenumb == platenumber then
...
RegisterNetEvent('tac_garbage:countbagtotal')
AddEventHandler('tac_garbage:countbagtotal', function(platenumber)
	if isInService then
		if platenumb == platenumber then
			if not iscurrentboss then
			TriggerServerEvent('tac_garbage:bagsdone', platenumb, totalbagpay)
			totalbagpay = 0
			end
		end
	end
...
RegisterNetEvent('tac_garbage:clearjob')
AddEventHandler('tac_garbage:clearjob', function(platenumber)
	if platenumb == platenumber then
...
function MenuVehicleSpawner()
	...
	tac.UI.Menu.Open(
		...
		function(data, menu)
			tac.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				if data.current.value == 'phantom3' then
					tac.Game.SpawnVehicle("trailers2", Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(trailer)
...
AddEventHandler('tac_garbage:hasEnteredMarker', function(zone)
...
AddEventHandler('tac_garbage:hasExitedMarker', function(zone)
	tac.UI.Menu.CloseAll()
    CurrentAction = nil
	CurrentActionMsg = ''
end)
...
function nouvelledestination()
	...
	TriggerServerEvent('tac_garbage:requestpay', platenumb,  temppayamount)
...
function retourcamion_non()
	...
	if livraisonnombre >= Config.MaxDelivery then
		tac.ShowNotification(_U('need_it'))
	else
		tac.ShowNotification(_U('ok_work'))
		nouvelledestination()
	end
end
...
function retourcamionperdu_non()
	tac.ShowNotification(_U('scared_me'))
end
...
function retourcamionannulermission_non()
	tac.ShowNotification(_U('resume_delivery'))
end
...
function SelectBinandCrew()
	...
	local NewBin, NewBinDistance = tac.Game.GetClosestObject(Config.DumpstersAvaialbe)
	trashcollectionpos = GetEntityCoords(NewBin)
	platenumb = GetVehicleNumberPlateText(GetVehiclePedIsIn(GetPlayerPed(-1), true))
	TriggerServerEvent("tac_garbage:binselect", trashcollectionpos, platenumb, bagsoftrash)
end
				...
				if CollectionAction == 'collection' then
					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							TriggerServerEvent('tac_garbage:bagremoval', platenumb)
							trashcollection = false
							Citizen.Wait(4000)
							...
							if iscurrentboss then
								...
								TriggerServerEvent('tac_garbage:reportbags', platenumb)
								Citizen.Wait(4000)
								ClearPedTasks(PlayerPedId())
        	        	        setring = false
            	        	    bagsoftrash = math.random(2,10)
								...
								trashcollection = false
								truckdeposit = false
								tac.ShowNotification("Trabalho terminado retornar ao caminhão!")
								while not IsPedInVehicle(GetPlayerPed(-1), work_truck, false) do
									Citizen.Wait(0)
								end
								TriggerServerEvent('tac_garbage:endcollection', platenumb)
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
			if dist <= 2.0 then
			tac.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "[~g~E~s~] Jogue o saco no caminhão.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag <= 0 then
					if iscurrentboss then
					tac.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Limpar detritos", 1.0)
					else
					tac.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Coleção feita .. Espere no caminhão..", 1.0)
					end
				else
					tac.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Coletar saco da lixeira ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
		...
		if not paused then
			...
			if IsJobgarbage() then
				...
				if isInMarker and not hasAlreadyEnteredMarker then
					...
					TriggerEvent('tac_garbage:hasEnteredMarker', currentZone)
				end

				if not isInMarker and hasAlreadyEnteredMarker then
					hasAlreadyEnteredMarker = false
					TriggerEvent('tac_garbage:hasExitedMarker', lastZone)
				end
			end
		end
...
function MissionLivraisonLetsGo()
	...
	if MissionRegion == 1 then -- Los santos
		tac.ShowNotification(_U('meet_ls'))
	elseif MissionRegion == 2 then -- Blaine County
		tac.ShowNotification(_U('meet_bc'))
	elseif MissionRegion == 0 then -- au cas ou
		tac.ShowNotification(_U('meet_del'))
	end
...
function MissionLivraisonStopRetourDepot()
	...
	tac.ShowNotification(_U('return_depot'))
...
Citizen.CreateThread(function()
	while true do
		...
		while debugit do
			Citizen.Wait(0)
			plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
			tac.Game.Utils.DrawText3D(plyCoords + vector3(0.0, 0.0, 0.1), "ID do caminhão de trabalho: "..tostring(work_truck), 0.8)
			tac.Game.Utils.DrawText3D(plyCoords + vector3(0.0, 0.0, 0.0), "Ação para verificar o pressionamento de botão: "..tostring(CollectionAction), 0.8)
			tac.Game.Utils.DrawText3D(plyCoords + vector3(0.0, 0.0, -0.1), "Depósito do caminhão: "..tostring(truckdeposit), 0.8)
			tac.Game.Utils.DrawText3D(plyCoords + vector3(0.0, 0.0, -0.2), "Coleção Bin: "..tostring(trashcollection), 0.8)
		end
	end
end)
