...
RegisterCommand('10', function(source, args, rawCommand)
	if PlayerData.job.name == 'police' or  PlayerData.job.name == 'offpolice' then
		...
		local xPlayer = tac.GetPlayerData()
		if code == "7" then
			TriggerServerEvent('tac_10system:setPolice', false)
		elseif code == "8" then
			TriggerServerEvent('tac_10system:setPolice', true)
		end
		TriggerServerEvent('tac_10system:SendStatus', code)
	end
	if PlayerData.job.name == 'offambulance' or PlayerData.job.name == 'ambulance' then
		code = rawCommand:sub(4)
		local xPlayer = tac.GetPlayerData()
		if code == "7" then
			TriggerServerEvent('tac_10system:setAmbulance', false)
		elseif code == "8" then
			TriggerServerEvent('tac_10system:setAmbulance', true)
		end
		TriggerServerEvent('tac_10system:SendStatus', code)
	end
end, false)
...
RegisterNetEvent('tac_10system:UpdateMessages')
AddEventHandler('tac_10system:UpdateMessages', function(parsedMessage, id)
	local added = false
...
RegisterNetEvent('tac_10system:clearSystem')
AddEventHandler('tac_10system:clearSystem', function(id)
	statuses = {
