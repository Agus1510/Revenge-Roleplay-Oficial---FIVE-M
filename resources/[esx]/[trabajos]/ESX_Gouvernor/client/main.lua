	...
	tac.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'citizen_wear' then
				...
				TriggerServerEvent('tac_gouverneur:removeWeapon','WEAPON_COMBATPISTOL')
			...
			if data.current.value == 'weapon_wear' then
				TriggerServerEvent('tac_gouverneur:giveWeapon','WEAPON_COMBATPISTOL', 450)
			end
...
function OpenHelicoSpawnerMenu()
	...
	local elements = {
		{label = 'Helicoptero', value = 'vehicle_list'},
	}
						...
						tac.TriggerServerCallback('tac_society:getVehiclesInGarage', function(vehicles)
							...
							tac.UI.Menu.Open(
								...
								function(data, menu)
									...
									tac.Game.SpawnVehicle(vehicleProps.model, Config.Zones.HelicoSpawnPoint.Pos, 291.0, function(vehicle)
										tac.Game.SetVehicleProperties(vehicle, vehicleProps)
										local playerPed = GetPlayerPed(-1)
										TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
									end)

									TriggerServerEvent('tac_society:removeVehicleFromGarage', 'gouvernor', vehicleProps)
							...
							function(data, menu)
								for i=1, #elements, 1 do							
									if Config.MaxInService == -1 then
										tac.Game.SpawnVehicle(data.current.value, Config.Zones.HelicoSpawnPoint.Pos, 291.0, function(vehicle) -- direction du spawn
											local playerPed = GetPlayerPed(-1)
										...
										tac.TriggerServerCallback('tac_service:enableService', function(canTakeService, maxInService, inServiceCount)
											if canTakeService then
												tac.Game.SpawnVehicle(data.current.value, Config.Zones.HelicoSpawnPoint.Pos, 291.0, function(vehicle)
													local playerPed = GetPlayerPed(-1)
													SetVehicleColours(vehicle, 12, 12)
													SetVehicleWindowTint(vehicle, 1)
												end)
											else
												tac.ShowNotification('Service complet : ' .. inServiceCount .. '/' .. maxInService)
											end
										end, 'gouvernor')
										break
									end
...
function OpenVehicleSpawnerMenu()
	...
	tac.UI.Menu.Open(
			...
			if data.current.value == 'vehicle_list' then
				...
				if Config.EnableSocietyOwnedVehicles then
						...
						tac.UI.Menu.Open(
							'default', GetCurrentResourceName(), 'spawn_vehicle',
							{
								title    = 'Vehiculos de servicio',
								elements = elements
							},
							function(data, menu)
								for i=1, #elements, 1 do							
									if Config.MaxInService == -1 then
										tac.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 180.0, function(vehicle) -- direction du spawn
											local playerPed = GetPlayerPed(-1)
											SetVehicleColours(vehicle, 12, 12)
											SetVehicleWindowTint(vehicle, 1)
										...
										tac.TriggerServerCallback('tac_service:enableService', function(canTakeService, maxInService, inServiceCount)
											if canTakeService then
												tac.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 180.0, function(vehicle)
													local playerPed = GetPlayerPed(-1)
													SetVehicleColours(vehicle, 12, 12)
													SetVehicleWindowTint(vehicle, 1)
													SetVehicleMod(vehicle, 11, 3, true)
		...
		function(data, menu)
			...
			CurrentActionMsg  = 'Presiona ~INPUT_CONTEXT~ para acceder al menu.'
			CurrentActionData = {}
		end
	)
end
...
function OpenGouvernorMenu()
	TriggerEvent('tac_society:openBossMenu', 'gouvernor', function(data, menu)
		menu.close()
	end)
end
...
function OpenTpGarage()
	...
	local spawnCoords = {
		...
		tac.Game.Teleport(playerPed, spawnCoords, function()
...
function OpenTpGarageVehicle()
	local playerPed = GetPlayerPed(-1)
		...
		tac.Game.Teleport(playerPed, spawnCoords, function()
			TriggerEvent('instance:close')
			tac.Game.SpawnVehicle(vehicleProps.model, spawnCoords, Config.Zones.VehicleSpawnExterior.Heading, function(vehicle)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					tac.Game.SetVehicleProperties(vehicle, vehicleProps)
				end)
			end)
...
function OpenTpGuest()
	TriggerServerEvent("tac_gouverneur:sendSonnette")
end
...
AddEventHandler('tac_gouverneur:hasEnteredMarker', function(zone)
...
AddEventHandler('tac_gouverneur:hasExitedMarker', function(zone)
	tac.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("tac_gouverneur:addPlayer", playerJob)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('tac_gouverneur:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tac_gouverneur:hasExitedMarker', LastZone)
		end
			...
			if IsControlPressed(0,  Keys['E']) and (GetGameTimer() - GUI.Time) > 300 and PlayerData.job ~= nil then
				...
				if CurrentAction == 'delete_helico' and PlayerData.job.name == 'gouvernor' then
							if Config.EnableSocietyOwnedVehicles then
								...
								TriggerServerEvent('tac_society:putVehicleInGarage', 'gouvernor', vehicleProps)
							else
								if
									GetEntityModel(vehicle) == GetHashKey('Buzzard2') or
									GetEntityModel(vehicle) == GetHashKey('Frogger2')
									then
									TriggerServerEvent('tac_service:disableService', 'gouvernor')
								end
							end
...
RegisterNetEvent("tac_gouverneur:sendRequest")
AddEventHandler("tac_gouverneur:sendRequest", function(name,id)
	stopRequest = true
	...
	while not stopRequest do
		...
		if(IsControlJustPressed(1, 23)) then
			TriggerServerEvent("tac_gouverneur:sendStatusToPoeple", id, 1)
			stopRequest = true
		end

		if(IsControlJustPressed(1, 47)) then
			TriggerServerEvent("tac_gouverneur:sendStatusToPoeple", id,0)
			stopRequest = true
		end
	end
end)

RegisterNetEvent("tac_gouverneur:sendStatus")
AddEventHandler("tac_gouverneur:sendStatus", function(status)
	if(status == 1) then
