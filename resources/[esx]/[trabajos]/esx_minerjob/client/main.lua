...
AddEventHandler("tac_miner:washing", function()
    Washing()
end)

RegisterNetEvent("tac_miner:remelting")
AddEventHandler("tac_miner:remelting", function()
    Remelting()
end)

RegisterNetEvent('tac_miner:timer')
AddEventHandler('tac_miner:timer', function()
    local timer = 0
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 5 then
                Draw3DText( Config.WashingX, Config.WashingY, Config.WashingZ+0.5 -1.400, ('Lavando piedras ' .. timer .. '%'), 4, 0.1, 0.1)
            end
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 5 then
                Draw3DText( Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ+0.5 -1.400, ('Fundiendo piedras ' .. timer .. '%'), 4, 0.1, 0.1)
            end
            if timer == 100 then
                timer = 0
                break
...
RegisterNetEvent('tac_miner:createblips')
AddEventHandler('tac_miner:createblips', function()
    Citizen.CreateThread(function()
        while true do 
            Citizen.Wait(1)
                if blips == true and blipActive == false then
                    ...
                    BeginTextCommandSetBlipName("STRING")
                    AddTextComponentString("Lavar piedras")
                    ...
                    BeginTextCommandSetBlipName("STRING")
                    AddTextComponentString("Vender piedras")
...
Citizen.CreateThread(function()
    ...
    SetBlipAsShortRange(blip1, true)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Vestidor mineros")
    EndTextCommandSetBlipName(blip1)   
end)
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.CloakroomX, Config.CloakroomY, Config.CloakroomZ, true) < 25 then
                    DrawMarker(20, Config.CloakroomX, Config.CloakroomY, Config.CloakroomZ, 0, 0, 0, 0, 0, 90.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
                        if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.CloakroomX, Config.CloakroomY, Config.CloakroomZ, true) < 1 then
                            tac.ShowHelpNotification('Presiona ~INPUT_CONTEXT~ para acceder al vestidor.')
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), locations[i].x, locations[i].y, locations[i].z, true) < 25 and mineActive == false then
                DrawMarker(20, locations[i].x, locations[i].y, locations[i].z, 0, 0, 0, 0, 0, 100.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
                    if GetDistanceBetweenCoords(GetEntityCoords(ped), locations[i].x, locations[i].y, locations[i].z, true) < 1 then
                        tac.ShowHelpNotification("Presiona ~INPUT_CONTEXT~ para empezar a minar.")
                            if IsControlJustReleased(1, 51) then
                                Animation()
                                mineActive = true
                            end
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 25 and washingActive == false then
            DrawMarker(20, Config.WashingX, Config.WashingY, Config.WashingZ, 0, 0, 0, 0, 0, 55.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
                if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.WashingX, Config.WashingY, Config.WashingZ, true) < 1 then
                    tac.ShowHelpNotification("Presiona ~INPUT_CONTEXT~ para lavar piedras.")
                        if IsControlJustReleased(1, 51) then
                            TriggerServerEvent("tac_miner:washing")
                end
            end
        end
    end
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 25 and remeltingActive == false then
            DrawMarker(20, Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, 0, 0, 0, 0, 0, 55.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 0, 0, 2, 0, 0, 0, 0)
                if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ, true) < 1 then
                    tac.ShowHelpNotification("Presiona ~INPUT_CONTEXT~ para fundir piedras.")
                        if IsControlJustReleased(1, 51) then
                          TriggerServerEvent("tac_miner:remelting")  
                    end
                end
            end
        end
	...
	local ped = PlayerPedId()
        Citizen.Wait(1)
            if GetDistanceBetweenCoords(GetEntityCoords(ped), Config.SellX, Config.SellY, Config.SellZ, true) < 2 then
                tac.ShowHelpNotification("Presiona ~INPUT_CONTEXT~ para vender las piedras.")
                    if IsControlJustReleased(1, 51) then
                        Jeweler()                          
            end
        end
...
function Cloakroom()
    local elements = {
        ...
        {label = 'Auto de trabajo',       value = 'vehicle'},
    }

    tac.UI.Menu.CloseAll()

    tac.UI.Menu.Open('default', GetCurrentResourceName(), 'miner_actions', {
        title    = 'Minero',
        align    = 'top-left',
        elements = elements
    }, function(data, menu)
        if data.current.value == 'cloakroom1' then
            ...
            blips = false
            blipActive = false
            TriggerEvent("tac_miner:createblips")
        ...
        elseif data.current.value == 'vehicle' then
            ...
            tac.ShowNotification("The vehicle was pulled out of the garage.")
        end
    end)
end
...
function Jeweler()
    ...
    tac.UI.Menu.Open('default', GetCurrentResourceName(), 'jeweler_actions', {
        title    = 'Joyero - venta',
        align    = 'top-left',
        elements = elements
    }, function(data, menu)
        if data.current.value == 'diamonds' then
            menu.close()
            TriggerServerEvent("tac_miner:selldiamond")
        elseif data.current.value == 'gold' then
            menu.close()
            TriggerServerEvent("tac_miner:sellgold")
        elseif data.current.value == 'iron' then
            menu.close()
            TriggerServerEvent("tac_miner:selliron")
        elseif data.current.value == 'copper' then
            menu.close()
            TriggerServerEvent("tac_miner:sellcopper")
        end
    end)
end
    ...
    Citizen.CreateThread(function()
		...
		local ped = PlayerPedId()	
                ...
                if impacts == 5 then
                    ...
                    mineActive = false
                    impacts = 0
                    TriggerServerEvent("tac_miner:givestone")
                    break
                end        
        end
    end)
