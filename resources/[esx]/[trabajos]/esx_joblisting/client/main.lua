...
function ShowJobListingMenu()
	tac.TriggerServerCallback('tac_joblisting:getJobsList', function(jobs)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'joblisting', {
			title    = _U('job_center'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			TriggerServerEvent('tac_joblisting:setJob', data.current.job)
			tac.ShowNotification(_U('new_job'))
			menu.close()
...
AddEventHandler('tac_joblisting:hasExitedMarker', function(zone)
	...
	while true do
		...
		for i=1, #Config.Zones, 1 do
			...
			if distance < (Config.ZoneSize.x / 2) then
				isInMarker = true
				tac.ShowHelpNotification(_U('access_job_center'))
			end
		end
		...
		if not isInMarker and hasAlreadyEnteredMarker then
			...
			TriggerEvent('tac_joblisting:hasExitedMarker')
		end
	end
end)
