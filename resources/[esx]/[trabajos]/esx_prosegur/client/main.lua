...
function StartNPCJob()
	...
	tac.ShowNotification(_U('GPS_info'))
	Done = true
end
...
function StopNPCJob(cancel)
	...
	if cancel then
		tac.ShowNotification(_U('cancel_mission'))
	else
		TriggerServerEvent('tac_brinks:GiveItem')
		StartNPCJob()
		Done = true
	end
end
...
function CloakRoomMenu()
	...
	function(data, menu)
		if data.current.value == 'citizen_wear' then
			...
			CreateBlip()
			menu.close()
			tac.ShowNotification(_U('end_service_notif'))
		...
		if data.current.value == 'job_wear' then
			...
			CreateBlip()
			menu.close()
			tac.ShowNotification(_U('take_service_notif'))
			tac.ShowNotification(_U('start_job'))
...
function VehicleMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
	...
	function(data, menu)
  		for i = 1, #elements, 1 do
			...
			tac.Game.SpawnVehicle(data.current.value.Hash, coords, Heading, function(vehicle)
				TaskWarpPedIntoVehicle(playerPed, vehicle, - 1)
				SetVehicleNumberPlateText(vehicle, platePrefix .. platenum)
				plate = GetVehicleNumberPlateText(vehicle)
				plate = string.gsub(plate, " ", "")
				name = 'Véhicule de ' .. platePrefix
				TriggerServerEvent('tac_vehiclelock:registerkeyjob', name, plate, 'no')
			end)
		break
		end
	menu.close()
...
AddEventHandler('tac_brinks:hasEnteredMarker', function(zone)
...
AddEventHandler('tac_brinks:hasExitedMarker', function(zone)
	if zone == 'Sell' then
		TriggerServerEvent('tac_brinks:stopSell')
	end
	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('tac_brinks:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('tac_brinks:hasExitedMarker', LastZone)
			end
		end
	end
end)
			...
			if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
				...
				if PlayerData.job.name == Config.nameJob then
					...
					if CurrentAction == 'Sell' then
						TriggerServerEvent('tac_brinks:startSell')
					end
						...
						if string.find (plate, platePrefix) then
							...
							if hash == GetHashKey(truck.Hash) then
								if GetVehicleEngineHealth(vehicle) <= 500 or GetVehicleBodyHealth(vehicle) <= 500 then
									tac.ShowNotification(_U('vehicle_broken'))
								else
									TriggerServerEvent('tac_vehiclelock:vehjobSup', plate, 'no')
									DeleteVehicle(vehicle)
								end
							end
						else
							tac.ShowNotification(_U('bad_vehicle'))
						end
					end
					CurrentAction = nil
				end
		...
		if IsControlJustReleased(1, Keys["DELETE"]) and PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
			if Onjob then
				...
				if IsPedInAnyVehicle(playerPed, false) and IsVehicleModel(GetVehiclePedIsIn(playerPed, false), GetHashKey("stockade")) then
					...
					tac.ShowNotification(_U('not_good_veh'))
				end
			end
		end
	end
