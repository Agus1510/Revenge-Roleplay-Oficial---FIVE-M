...
function StartNPCJob()
  ...
  tac.ShowNotification(_U('drive_to_indicated'))
end
...
function StopNPCJob(cancel)
  ...
  if cancel then
    tac.ShowNotification(_U('mission_canceled'))
  else
    TriggerServerEvent('tac_mecanojob:onNPCJobCompleted')
  end
  ...
  tac.UI.Menu.Open(
    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            tac.TriggerServerCallback('tac_society:getVehiclesInGarage', function(vehicles)
              ...
              tac.UI.Menu.Open(
                ...
                function(data, menu)
                  ...
                  tac.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
                    tac.Game.SetVehicleProperties(vehicle, vehicleProps)
                    local playerPed = GetPlayerPed(-1)
                    TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
                  end)

                  TriggerServerEvent('tac_society:removeVehicleFromGarage', 'mecano', vehicleProps)
            ...
            local elements = {
			  ...
			  {label = _U('Honda Acty'), value = 'acty'}
            }
            ...
            if Config.EnablePlayerManagement and PlayerData.job ~= nil and
              ...
              table.insert(elements, {label = 'Honda acty', value = 'acty'})
            end
              ...
              function(data, menu)
                for i=1, #elements, 1 do
                  if Config.MaxInService == -1 then
                    tac.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 90.0, function(vehicle)
                      local playerPed = GetPlayerPed(-1)
                      TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
                    end)
                    break
                    ...
                    tac.TriggerServerCallback('tac_service:enableService', function(canTakeService, maxInService, inServiceCount)
                      if canTakeService then
                        ...
                        tac.ShowNotification(_U('service_full') .. inServiceCount .. '/' .. maxInService)
                      end
                    end, 'mecano')
                    break
                  end
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('tac_society:openBossMenu', 'mecano', function(data, menu)
          menu.close()
        end)
      end
...
function OpenMecanoHarvestMenu()
  ...
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    tac.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'gaz_bottle' then
          menu.close()
          TriggerServerEvent('tac_mecanojob:startHarvest')
        end

        if data.current.value == 'fix_tool' then
          menu.close()
          TriggerServerEvent('tac_mecanojob:startHarvest2')
        end

        if data.current.value == 'caro_tool' then
          menu.close()
          TriggerServerEvent('tac_mecanojob:startHarvest3')
        end
    ...
    tac.ShowNotification(_U('not_experienced_enough'))
  end
end
    ...
    tac.UI.Menu.Open(
      ...
      function(data, menu)
        if data.current.value == 'blow_pipe' then
          menu.close()
          TriggerServerEvent('tac_mecanojob:startCraft')
        end

        if data.current.value == 'fix_kit' then
          menu.close()
          TriggerServerEvent('tac_mecanojob:startCraft2')
        end

        if data.current.value == 'caro_kit' then
          menu.close()
          TriggerServerEvent('tac_mecanojob:startCraft3')
        end
          ...
          function(data, menu)
            ...
            if amount == nil then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('tac_billing:sendBill', GetPlayerServerId(closestPlayer), 'bank', _U('mechanic'), amount)
              end
            end
          end,
        function(data, menu)
        ...
        if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
          ...
          if DoesEntityExist(vehicle) then
            ...
            Citizen.CreateThread(function()
              ...
              ClearPedTasksImmediately(playerPed)
              tac.ShowNotification(_U('vehicle_repaired'))
            end)
          end
        end
      end
        ...
        if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
          ...
          if DoesEntityExist(vehicle) then
            ...
            Citizen.CreateThread(function()
              ...
              ClearPedTasksImmediately(playerPed)
              tac.ShowNotification(_U('vehicle_cleaned'))
            end)
          end
        end
      end
        ...
        if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then
          ...
          if ( IsPedSittingInAnyVehicle( ped ) ) then
            ...
            if ( GetPedInVehicleSeat( vehicle, -1 ) == ped ) then
              tac.ShowNotification(_U('vehicle_impounded'))
              SetEntityAsMissionEntity( vehicle, true, true )
              deleteCar( vehicle )
            else
              tac.ShowNotification(_U('must_seat_driver'))
            end
          else
            ...
            if ( DoesEntityExist( vehicle ) ) then
              ...
              tac.ShowNotification(_U('must_near'))
            end
          end
        end
      end
          ...
          if CurrentlyTowedVehicle == nil then
            if targetVehicle ~= 0 then
              if not IsPedInAnyVehicle(playerped, true) then
                if vehicle ~= targetVehicle then
                  ...
                  tac.ShowNotification(_U('vehicle_success_attached'))

                  if NPCOnJob then

                    if NPCTargetTowable == targetVehicle then
                      tac.ShowNotification(_U('please_drop_off'))
                  ...
                  tac.ShowNotification(_U('cant_attach_own_tt'))
                end
              end
            else
              tac.ShowNotification(_U('no_veh_att'))
            end
          else
            ...
            if NPCOnJob then
              ...
              if CurrentlyTowedVehicle == NPCTargetTowable then
                tac.Game.DeleteVehicle(NPCTargetTowable)
                TriggerServerEvent('tac_mecanojob:onNPCJobMissionCompleted')
                StopNPCJob()

              else
                tac.ShowNotification(_U('not_right_veh'))
              end
            ...
            tac.ShowNotification(_U('veh_det_succ'))
          end
        else
          tac.ShowNotification(_U('imp_flatbed'))
        end
      end
...
function OpenGetStocksMenu()
  ...
  tac.TriggerServerCallback('tac_mecanojob:getStockItems', function(items)
      ...
      function(data, menu)
        ...
        tac.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              tac.ShowNotification(_U('invalid_quantity'))
            else
              menu2.close()
              menu.close()
              OpenGetStocksMenu()

              TriggerServerEvent('tac_mecanojob:getStockItem', itemName, count)
            end
...
tac.TriggerServerCallback('tac_mecanojob:getPlayerInventory', function(inventory)
        ...
        tac.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('tac_mecanojob:putStockItems', itemName, count)
            end
...
RegisterNetEvent('tac_mecanojob:onHijack')
AddEventHandler('tac_mecanojob:onHijack', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
    ...
    if DoesEntityExist(vehicle) then
      ...
      Citizen.CreateThread(function()
        ...
        if crochete <= 66 then
          ...
          SetVehicleDoorsLockedForAllPlayers(vehicle, false)
          ClearPedTasksImmediately(playerPed)
          tac.ShowNotification(_U('veh_unlocked'))
        else
          tac.ShowNotification(_U('hijack_failed'))
          ClearPedTasksImmediately(playerPed)
        end
      end)
    end
...
RegisterNetEvent('tac_mecanojob:onCarokit')
AddEventHandler('tac_mecanojob:onCarokit', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
  ...
  if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
    ...
    if DoesEntityExist(vehicle) then
      ...
      Citizen.CreateThread(function()
        ...
        SetVehicleDeformationFixed(vehicle)
        ClearPedTasksImmediately(playerPed)
        tac.ShowNotification(_U('body_repaired'))
      end)
    end
  end
end)

RegisterNetEvent('tac_mecanojob:onFixkit')
AddEventHandler('tac_mecanojob:onFixkit', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
  ...
  if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
    ...
    if DoesEntityExist(vehicle) then
      ...
      Citizen.CreateThread(function()
        ...
        SetVehicleUndriveable(vehicle, false)
        ClearPedTasksImmediately(playerPed)
        tac.ShowNotification(_U('veh_repaired'))
      end)
    end
  end
end)
...
AddEventHandler('tac_mecanojob:hasEnteredMarker', function(zone)
...
AddEventHandler('tac_mecanojob:hasExitedMarker', function(zone)

  if zone == 'Craft' then
    TriggerServerEvent('tac_mecanojob:stopCraft')
    TriggerServerEvent('tac_mecanojob:stopCraft2')
    TriggerServerEvent('tac_mecanojob:stopCraft3')
  end

  if zone == 'Garage' then
    TriggerServerEvent('tac_mecanojob:stopHarvest')
    TriggerServerEvent('tac_mecanojob:stopHarvest2')
    TriggerServerEvent('tac_mecanojob:stopHarvest3')
  end
...
AddEventHandler('tac_mecanojob:hasEnteredEntityZone', function(entity)
...
AddEventHandler('tac_mecanojob:hasExitedEntityZone', function(entity)
  ...
  while true do
    ...
    if NPCTargetTowableZone ~= nil and not NPCHasSpawnedTowable then
      ...
      if GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < Config.NPCSpawnDistance then
        ...
        tac.Game.SpawnVehicle(model, zone.Pos, 0, function(vehicle)
    ...
    if NPCTargetTowableZone ~= nil and NPCHasSpawnedTowable and not NPCHasBeenNextToTowable then
      ...
      if(GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < Config.NPCNextToDistance) then
        tac.ShowNotification(_U('please_tow'))
  ...
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
      ...
      if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
        ...
        TriggerEvent('tac_mecanojob:hasEnteredMarker', currentZone)
      end
      if not isInMarker and HasAlreadyEnteredMarker then
        HasAlreadyEnteredMarker = false
        TriggerEvent('tac_mecanojob:hasExitedMarker', LastZone)
      end
    end
  end
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    local entity, distance = tac.Game.GetClosestObject({
      'prop_roadcone02a',
      'prop_toolchest_01'
    })
    ...
    if distance ~= -1 and distance <= 3.0 then
      ...
      if LastEntity ~= entity then
        TriggerEvent('tac_mecanojob:hasEnteredEntityZone', entity)
        LastEntity = entity
      end
      ...
      if LastEntity ~= nil then
        TriggerEvent('tac_mecanojob:hasExitedEntityZone', LastEntity)
        LastEntity = nil
      end
          ...
          if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
            ...
            if CurrentAction == 'delete_vehicle' then
              ...
              if Config.EnableSocietyOwnedVehicles then
                ...
                TriggerServerEvent('tac_society:putVehicleInGarage', 'mecano', vehicleProps)
                ...
                then
                  TriggerServerEvent('tac_service:disableService', 'mecano')
                end
        ...
        if IsControlJustReleased(0, Keys['DELETE']) and PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
          ...
          if NPCOnJob then
            ...
            if GetGameTimer() - NPCLastCancel > 5 * 60000 then
              ...
              tac.ShowNotification(_U('wait_five'))
            end
            ...
            if IsPedInAnyVehicle(playerPed,  false) and IsVehicleModel(GetVehiclePedIsIn(playerPed,  false), GetHashKey("flatbed")) then
              ...
              tac.ShowNotification(_U('must_in_flatbed'))
            end
