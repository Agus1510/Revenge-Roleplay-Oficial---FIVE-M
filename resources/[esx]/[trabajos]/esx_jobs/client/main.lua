...
AddEventHandler('tac_jobs:action', function(job, zone, zoneIndex)
	menuIsShowed = true
	...
	elseif zone.Type == 'work' then
		...
		if IsPedOnFoot(playerPed) then
			TriggerServerEvent('tac_jobs:startWork', zoneIndex)
		else
			tac.ShowNotification(_U('foot_work'))
		end
	elseif zone.Type == 'vehspawner' then
		...
		if jobObject and spawnPoint and vehicle and tac.Game.IsSpawnPointClear(spawnPoint.Pos, 5.0) then
			spawnVehicle(spawnPoint, vehicle, zone.Caution)
		else
			tac.ShowNotification(_U('spawn_blocked'))
		end
			...
			for k,v in pairs(jobObject.Zones) do
				if v.Type == 'vehdelete' and v.Spawner == zone.Spawner then
					...
					if IsPedInAnyVehicle(playerPed, false) then
						...
						local plate = tac.Math.Trim(GetVehicleNumberPlateText(vehicle))
						...
						if playerPed == driverPed then
							if myPlate[plate] then
								...
								local giveBack = tac.Math.Round(vehicleHealth / vehicleMaxHealth, 2)

								TriggerServerEvent('tac_jobs:caution', 'give_back', giveBack, 0, 0)
								DeleteVehicle(GetVehiclePedIsIn(playerPed, false))

								if v.Teleport ~= 0 then
									tac.Game.Teleport(playerPed, v.Teleport)
								end
							...
							tac.ShowNotification(_U('not_your_vehicle'))
						end
...
function nextStep(gps)
	if gps ~= 0 then
		...
		tac.ShowNotification(_U('next_point'))
	end
end

AddEventHandler('tac_jobs:hasExitedMarker', function(zone)
	TriggerServerEvent('tac_jobs:stopWork')
	hintToDisplay = nil
	menuIsShowed = false
	hintIsShowed = false
	isInMarker = false
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	hintToDisplay = nil
	hintIsShowed = false
	TriggerServerEvent('tac_jobs:caution', 'take', vehicleCaution, spawnPoint, vehicle)
end

RegisterNetEvent('tac_jobs:spawnJobVehicle')
AddEventHandler('tac_jobs:spawnJobVehicle', function(spawnPoint, vehicle)
	local playerPed = PlayerPedId()

	tac.Game.SpawnVehicle(vehicle.Hash, spawnPoint.Pos, spawnPoint.Heading, function(spawnedVehicle)

		if vehicle.Trailer ~= 'none' then
			tac.Game.SpawnVehicle(vehicle.Trailer, spawnPoint.Pos, spawnPoint.Heading, function(trailer)
...
Citizen.CreateThread(function()
	while true do
		...
		if hintIsShowed and hintToDisplay then
			tac.ShowHelpNotification(hintToDisplay)
		else
			Citizen.Wait(500)
		end
	end
	...
	while true do
		...
		if PlayerData.job then
			...
			if IsControlJustReleased(0, 38) and not menuIsShowed and isInMarker then
				if onDuty or currentZone.Type == 'cloakroom' or PlayerData.job.name == 'reporter' then
					TriggerEvent('tac_jobs:action', PlayerData.job.name, currentZone, currentZoneIndex)
				end
			end
			...
			if not isInMarker and hasAlreadyEnteredMarker then
				...
				TriggerEvent('tac_jobs:hasExitedMarker', currentZone)
			end
		end
		...
		for k,v in pairs(Config.PublicZones) do
			...
			if distance < v.Size.x / 2 then
				...
				tac.ShowHelpNotification(v.Hint)
