...
AddEventHandler('tac_jobs:startWork', function(zoneIndex)
...
RegisterNetEvent('tac_jobs:caution')
		...
		if cautionAmount <= Config.MaxCaution and cautionAmount > 0 then
			TriggerEvent('tac_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
				if xPlayer.getAccount('bank').money >= cautionAmount then
					...
					xPlayer.showNotification(_U('bank_deposit_taken', tac.Math.GroupDigits(cautionAmount)))
					TriggerClientEvent('tac_jobs:spawnJobVehicle', xPlayer.source, spawnPoint, vehicle)
				else
					xPlayer.showNotification(_U('caution_afford', tac.Math.GroupDigits(cautionAmount)))
				end
			end)
		end
