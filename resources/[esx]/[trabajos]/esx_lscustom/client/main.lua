...
AddEventHandler('tac:playerLoaded', function(xPlayer)
	...
	tac.TriggerServerCallback('tac_lscustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('tac_lscustom:installMod')
AddEventHandler('tac_lscustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = tac.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('tac_lscustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('tac_lscustom:cancelInstallMod')
AddEventHandler('tac_lscustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	tac.Game.SetVehicleProperties(vehicle, myCar)
end)

function OpenLSMenu(elems, menuName, menuTitle, parent)
	tac.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		title    = menuTitle,
		align    = 'top-left',
		elements = elems
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					tac.ShowNotification(_U('already_own', data.current.label))
					TriggerEvent('tac_lscustom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('tac_lscustom:buyMod', price)
		...
		menu.close()
		TriggerEvent('tac_lscustom:cancelInstallMod')
...
function GetAction(data)
	...
	local playerPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(playerPed, false)
	local currentMods = tac.Game.GetVehicleProperties(vehicle)
