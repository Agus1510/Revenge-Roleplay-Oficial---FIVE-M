	...
	if len > 1 then
		...
		tac.UI.Menu.Open(
			...
			function(data, menu)
				...
				tac.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, Config.Zones.VehicleSpawnPoint.Heading, function(vehicle)
		...
		tac.Game.SpawnVehicle(Config.Vehicle[1], Config.Zones.VehicleSpawnPoint.Pos, Config.Zones.VehicleSpawnPoint.Heading, function(vehicle)
			vehicleMaxHealth = GetVehicleEngineHealth(vehicle)
			SetVehicleNumberPlateText(vehicle, plate)             			
			TaskWarpPedIntoVehicle(GetPlayerPed(-1), vehicle, -1)   
		end)
...
function MenuDistribution()
	...
	tac.UI.Menu.CloseAll()
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'distribution', {
		title    = _U('distribution'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)

		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'distribution_choice', {
...
AddEventHandler('gopostal_job:hasEnteredMarker', function(zone)
	...
	if zone == 'VehicleDeleter' and isInService then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			local vehicle, distance = tac.Game.GetClosestVehicle({
				x = coords.x,
				y = coords.y,
				z = coords.z
			})
...
function Livraison()
	...
	if CurrentDelivery then
		...
		tac.ShowAdvancedNotification(_U('notif_title_delivery'), '', _U('notif_district', district.label), 'CHAR_BRYONY', 1 )
		tac.ShowAdvancedNotification(_U('notif_title_delivery'), '', _U('create_itinary'), 'CHAR_ANDREAS', 1 )
...
function LivraisonStart(district)
	if CurrentDelivery then
		...
		LetterAndColis()
		tac.ShowNotification(_U('join_next'))
	end
end
...
function LivraisonStop(district, cancel)
	...
	if cancel then
		tac.ShowNotification(_U('cancel_delivery'))
		CurrentDelivery = false
	else
		if progress < district.long then
			...
			CurrentDelivery = false
			progress = 1
			tac.ShowAdvancedNotification(_U('notif_title_delivery'), '', _U('finish_delivery'), 'CHAR_BRYONY', 1 )
		end
	end
end
		      		...
		      		if GetDistanceBetweenCoords(coords, zone.Pos[progress].x, zone.Pos[progress].y, zone.Pos[progress].z, true) < 3.0 then
			        	...
			        	if IsControlJustReleased(0, 38) and IsJobTrucker() and isInService then
			        		if not IsPedInAnyVehicle(PlayerPedId(), false) then
			          			tac.TriggerServerCallback('gopostal_job:haveItem', function(haveItem)
			          				if haveItem then
			          			...
			          			tac.ShowNotification(_U('must_be_walking'))
			          		end
			        	end
			        end
		      	end
