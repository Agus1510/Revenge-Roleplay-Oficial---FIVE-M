...
AddEventHandler('gopostal_job:end', function(letter, colis)
	...
	TriggerClientEvent('tac:showAdvancedNotification', _source, 'GoPostal', '', _U('gain', tonumber(gainT)), 'CHAR_BRYONY', 1)

end)

tac.RegisterServerCallback('gopostal_job:haveItem', function(source, cb, letter, colis) -- Check if player have item
	local _source = source
	local xPlayer = tac.GetPlayerFromId(_source)
	...
	if lcount >= letter and ccount >= colis then
		...
		if lcount < letter then
			TriggerClientEvent('tac:showNotification', _source, _U('not_enought_letter', letter - lcount))
		end
		if ccount < colis then
			TriggerClientEvent('tac:showNotification', _source, _U('not_enought_colis', colis - ccount))
		end
	end
end) 
--------------------------------------------------------------------------------
...
AddEventHandler('gopostal_job:Item', function(itemName, amount, label, type)
	...
	if type == 'pick' then
		if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
			TriggerClientEvent('tac:showNotification', _source, _U('player_cannot_hold'))
		else
			xPlayer.addInventoryItem(itemName, amount)
			TriggerClientEvent('tac:showNotification', _source, _U('take', amount, label))
		end
	elseif type == 'deposit' then

		if sourceItem.count >= amount then
			xPlayer.removeInventoryItem(itemName, amount)
			TriggerClientEvent('tac:showNotification', _source, _U('remove', amount, label))
		else
...
AddEventHandler('gopostal_job:caution', function(cautionType, cautionAmount)
	...
	if cautionType == "take" then
		...
		TriggerClientEvent('tac:showNotification', source, _U('bank_deposit_taken', tac.Math.GroupDigits(cautionAmount)))
	...
	elseif cautionType == "give_back" then
		...
		local toGive = tac.Math.Round(caution * cautionAmount)

		xPlayer.addAccountMoney('bank', toGive)
		TriggerClientEvent('tac:showNotification', source, _U('bank_deposit_returned', tac.Math.GroupDigits(toGive)))
	end
end)
