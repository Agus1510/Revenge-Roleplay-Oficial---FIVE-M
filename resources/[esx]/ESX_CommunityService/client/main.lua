...
Citizen.CreateThread(function()
	...
	TriggerServerEvent('tac_communityservice:checkIfSentenced')
end)
...
RegisterNetEvent('tac_communityservice:inCommunityService')
AddEventHandler('tac_communityservice:inCommunityService', function(actions_remaining)
	...
	ApplyPrisonerSkin()
	tac.Game.Teleport(playerPed, Config.ServiceLocation)
	isSentenced = true
	...
	while actionsRemaining > 0 and communityServiceFinished ~= true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z) > 45 then
			tac.Game.Teleport(playerPed, Config.ServiceLocation)
				TriggerEvent('chat:addMessage', { args = { _U('judge'), _U('escape_attempt') }, color = { 249, 166, 0 } })
				TriggerServerEvent('tac_communityservice:extendService')
				actionsRemaining = actionsRemaining + Config.ServiceExtensionOnEscape
		end

	end

	TriggerServerEvent('tac_communityservice:finishCommunityService', -1)
	tac.Game.Teleport(playerPed, Config.ReleaseLocation)
	isSentenced = false
...
RegisterNetEvent('tac_communityservice:finishCommunityService')
AddEventHandler('tac_communityservice:finishCommunityService', function(source)
...
Citizen.CreateThread(function()
	while true do
		...
		if actionsRemaining > 0 and isSentenced then
			draw2dText( _U('remaining_msg', tac.Math.Round(actionsRemaining)), { 0.175, 0.955 } )
			...
			for i = 1, #availableActions do
				...
				if distance < 1.5 then
					...
					if(IsControlJustReleased(1, 38))then
						...
						TriggerServerEvent('tac_communityservice:completeService')
						...
						if (tmp_action.type == "cleaning") then
							...
							tac.Streaming.RequestAnimDict("amb@world_human_janitor@male@idle_a", function()
