...
RegisterNetEvent('tac_hifi:place_hifi')
AddEventHandler('tac_hifi:place_hifi', function()
    startAnimation("anim@heists@money_grab@briefcase","put_down_case")
    Citizen.Wait(1000)
    ClearPedTasks(PlayerPedId())
    TriggerEvent('tac:spawnObject', 'prop_boombox_01')
end)

RegisterNetEvent('tac_hifi:play_music')
AddEventHandler('tac_hifi:play_music', function(id, object)
...
RegisterNetEvent('tac_hifi:stop_music')
AddEventHandler('tac_hifi:stop_music', function(object)
    if distance(object) < Config.distance then
        SendNUIMessage({
            transactionType = 'stopSound'
        })
...
RegisterNetEvent('tac_hifi:setVolume')
AddEventHandler('tac_hifi:setVolume', function(volume, object)
    if distance(object) < Config.distance then
        SendNUIMessage({
...
function OpenhifiMenu()
    menuOpen = true
    tac.UI.Menu.Open('default', GetCurrentResourceName(), 'hifi', {
        ...
        if data.current.value == 'get_hifi' then
            ...
            for i=1, #tac.PlayerData.inventory, 1 do
                if tac.PlayerData.inventory[i].name == 'hifi' and tac.PlayerData.inventory[i].count > 0 then
                    alreadyOne = true
                end
            end
            if not alreadyOne then
                ...
                tac.Game.DeleteObject(currentData)
                if not DoesEntityExist(currentData) then
                    TriggerServerEvent('tac_hifi:remove_hifi', lCoords)
                    currentData = nil
                end
                Citizen.Wait(500)
                ClearPedTasks(PlayerPedId())
                ...
                menu.close()
                menuOpen = false
                TriggerEvent('tac:showNotification', _U('hifi_alreadyOne'))
            end
        elseif data.current.value == 'play' then
            play(lCoords)
        elseif data.current.value == 'stop' then
            TriggerServerEvent('tac_hifi:stop_music', lCoords)
            menuOpen = false
            menu.close()
...
function setVolume(coords)
    tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'setvolume',
        {
            title = _U('set_volume'),
        }, function(data, menu)
            local value = tonumber(data.value)
            if value < 0 or value > 100 then
                tac.ShowNotification(_U('sound_limit'))
            else
                TriggerServerEvent('tac_hifi:setVolume', value, coords)
                menu.close()
            end
        end, function(data, menu)
            menu.close()
...
function play(coords)
    tac.UI.Menu.Open('dialog', GetCurrentResourceName(), 'play',
        {
            title = _U('play'),
        }, function(data, menu)
            TriggerServerEvent('tac_hifi:play_music', data.value, coords)
            menu.close()
        end, function(data, menu)
            menu.close()
        end)
    ...
    while true do
        ...
        if closestDistance ~= -1 and closestDistance <= 3.0 then
            if lastEntity ~= closestEntity and not menuOpen then
                tac.ShowHelpNotification(_U('hifi_help'))
