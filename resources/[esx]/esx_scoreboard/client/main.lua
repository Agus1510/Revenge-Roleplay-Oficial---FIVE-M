...
Citizen.CreateThread(function()
	...
	Citizen.Wait(2000)
	tac.TriggerServerCallback('tac_scoreboard:getConnectedPlayers', function(connectedPlayers)
...
Citizen.CreateThread(function()
	...
	SendNUIMessage({
		...
		maxPlayers = GetConvarInt('sv_maxclients', 33),
		uptime = 'unknown',
		playTime = '00h 00m'
	})
end)

RegisterNetEvent('tac_scoreboard:updateConnectedPlayers')
AddEventHandler('tac_scoreboard:updateConnectedPlayers', function(connectedPlayers)
	UpdatePlayerTable(connectedPlayers)
end)

RegisterNetEvent('tac_scoreboard:updatePing')
AddEventHandler('tac_scoreboard:updatePing', function(connectedPlayers)
	SendNUIMessage({
		action  = 'updatePing',
...
RegisterNetEvent('tac_scoreboard:toggleID')
AddEventHandler('tac_scoreboard:toggleID', function(state)
	if state then
		idVisable = state
	else
